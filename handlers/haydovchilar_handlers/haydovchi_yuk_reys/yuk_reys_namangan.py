...
from states.yuk_reys_states import Yuk_reys_namangan


@dp.callback_query_handler(taxi_yuk_callback.filter(item_name='tuesday'))
async def reys(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_reys_namangan.tuman.set()
@dp.callback_query_handler(text='ortga',state=Yuk_reys_namangan.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Yuk_reys_namangan.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Yuk_reys_namangan.tuman)
async def reys_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_reys_namangan.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Yuk_reys_namangan.viloyatga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan ketasiz ? ", reply_markup=namangan_yol)#############
    await Yuk_reys_namangan.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_reys_namangan.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_reys_namangan.viloyatga)
async def reys_viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_reys_namangan.tumaniga.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_namangan.tumaniga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_reys_namangan.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_namangan.tumaniga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_reys_namangan.tumaniga)
async def reys_tumaniga(call: CallbackQuery, state: FSMContext):
    ...
    await Yuk_reys_namangan.kuni.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_namangan.kuni)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga yuk olib borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_reys_namangan.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_namangan.kuni)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Yuk_reys_namangan.kuni)
async def reys_kuni(message: Message, state: FSMContext):
    ...
    await Yuk_reys_namangan.soat.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_namangan.soat)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )", reply_markup=reys_ortgaa)
    await Yuk_reys_namangan.kuni.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_namangan.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\n"
...
@dp.callback_query_handler(state=Yuk_reys_namangan.soat)
async def reys_soat(call: CallbackQuery, state: FSMContext):
    ...
    await Yuk_reys_namangan.phone.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_namangan.phone)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ", reply_markup=time)
    await Yuk_reys_namangan.soat.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_namangan.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'], state=Yuk_reys_namangan.phone)
async def reys_loc(message: Message, state: FSMContext):
    ...
    await Yuk_reys_namangan.qanday_moshina.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_namangan.qanday_moshina)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Telefon raqamingizni kiriting ..", reply_markup=phone_number)
    await call.message.answer("Mana shu raqamni ishlatayotgan bo'lsangiz\n"
                              "Kontakt yuborish ni bosing", reply_markup=reys_ortgaa)
...
@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_namangan.qanday_moshina)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Yuk_reys_namangan.qanday_moshina)
async def reys_qanday_avto(message: Message, state: FSMContext):
    ...
    await Yuk_reys_namangan.tasdiqlash.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_namangan.tasdiqlash)
async def reys_ortga(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Mashinangiz qanday ? ")
    await Yuk_reys_namangan.qanday_moshina.set()


@dp.callback_query_handler(text='yesss', state=Yuk_reys_namangan.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Yuk_reys_namangan.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(text='add_information', state=Yuk_reys_namangan.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'l haqqi uchun qancha olasiz ? :   ")
    await Yuk_reys_namangan.xa_yoq.set()


@dp.message_handler(state=Yuk_reys_namangan.xa_yoq)
async def kisi(message: Message, state: FSMContext):
    ...
    await Yuk_reys_namangan.pochta_olasizmi.set()


@dp.message_handler(state=Yuk_reys_namangan.pochta_olasizmi)
async def reys_pochta_olasizmi(message: Message, state: FSMContext):
    ...
    await Yuk_reys_namangan.yuk_olasizmi.set()


@dp.message_handler(state=Yuk_reys_namangan.yuk_olasizmi)
async def yuk_olasizmi_reys(message: Message, state: FSMContext):
    ...
    await Yuk_reys_namangan.jami_odam.set()


@dp.message_handler(state=Yuk_reys_namangan.jami_odam)
async def jami_odam(message: Message, state: FSMContext):
    ...
    await Yuk_reys_namangan.odam.set()


@dp.message_handler(state=Yuk_reys_namangan.odam)
async def reys_odam(message: Message, state: FSMContext):
    ...
    await Yuk_reys_namangan.end.set()


@dp.callback_query_handler(text='Confirm', state=Yuk_reys_namangan.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Yuk_reys_namangan.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
