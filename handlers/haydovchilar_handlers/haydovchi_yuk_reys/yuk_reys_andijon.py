...
async def andi_jon(call: CallbackQuery):
...
async def sasassasas(call: CallbackQuery, state: FSMContext):
    ...
    await Yuk_reys_andijon.qoshimcha_tumanlarga.set()


@dp.callback_query_handler(state=Yuk_reys_andijon.qoshimcha_tumanlarga)
async def qoshimcha_tumanalarga(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == 'andijon':
        ...
        await Yuk_reys_andijon.qoshimcha_tuman.set()
        await call.message.delete()
...
@dp.callback_query_handler(text="ortga", state=Yuk_reys_andijon.qoshimcha_tumanlarga)
...
@dp.callback_query_handler(text="qoldayozish", state=Yuk_reys_andijon.jami_odam)
async def qolda_kiritish(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Nechta tonna yuk olishingizni yozma kiriting .")
    await call.message.delete()
    await Yuk_reys_andijon.qolda_odam_soni.set()
