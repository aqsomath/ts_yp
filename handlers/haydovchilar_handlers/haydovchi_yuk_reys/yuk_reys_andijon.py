...
@dp.callback_query_handler(taxi_yuk_callback.filter(item_name='monday'))
...
@dp.callback_query_handler(text='ortga',state=Yuk_reys_andijon.viloyatga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
...
@dp.message_handler(state=Yuk_reys_andijon.kuni)
...
@dp.callback_query_handler(text='ortga',state=Yuk_reys_andijon.phone)
async def andi_jon(call:CallbackQuery,state:FSMContext):
...
async def reys_loc(message:Message,state:FSMContext):
    ...
    await Yuk_reys_andijon.qanday_moshina.set()
@dp.callback_query_handler(text='ortga',state=Yuk_reys_andijon.qanday_moshina)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Telefon raqamingizni kiriting ..", reply_markup=phone_number)
    await call.message.answer("Mana shu raqamni ishlatayotgan bo'lsangiz\n"
                              "Kontakt yuborish ni bosing", reply_markup=reys_ortgaa)
...
@dp.callback_query_handler(text_contains='atmen',state=Yuk_reys_andijon.qanday_moshina)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.message_handler(state=Yuk_reys_andijon.qanday_moshina)
...
@dp.message_handler(state=Yuk_reys_andijon.xa_yoq)
...
@dp.message_handler(state=Yuk_reys_andijon.pochta_olasizmi)
...
@dp.message_handler(state=Yuk_reys_andijon.yuk_olasizmi)
...
@dp.message_handler(state=Yuk_reys_andijon.jami_odam)
async def jami_odam(message:Message,state:FSMContext):
    ...
    await Yuk_reys_andijon.odam.set()

@dp.message_handler(state=Yuk_reys_andijon.odam)
