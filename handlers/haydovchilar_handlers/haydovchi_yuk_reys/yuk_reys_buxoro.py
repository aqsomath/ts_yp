...
from handlers.users.edit_district.buxoro_edit import Olot, Buxoroshaxar, Gijduvon, Jondor, Kogon, Qorakoʻl, \
...
from states.yuk_reys_states import Yuk_reys_buxoro


@dp.callback_query_handler(taxi_yuk_callback.filter(item_name='thursday'))
async def reys(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Yukni qaysi tumandan olasiz ? ",reply_markup=buxoro_yol)
    await Yuk_reys_buxoro.tuman.set()
@dp.callback_query_handler(text='ortga',state=Yuk_reys_buxoro.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Yuk_reys_buxoro.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Yuk_reys_buxoro.tuman)
async def reys_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_reys_buxoro.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Yuk_reys_buxoro.viloyatga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan ketasiz ? ", reply_markup=buxoro_yol)#############
    await Yuk_reys_buxoro.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_reys_buxoro.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_reys_buxoro.viloyatga)
async def reys_viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_reys_buxoro.tumaniga.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_buxoro.tumaniga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_reys_buxoro.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_buxoro.tumaniga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_reys_buxoro.tumaniga)
async def reys_tumaniga(call: CallbackQuery, state: FSMContext):
    ...
    await Yuk_reys_buxoro.kuni.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_buxoro.kuni)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga yuk olib borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_reys_buxoro.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_buxoro.kuni)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Yuk_reys_buxoro.kuni)
async def reys_kuni(message: Message, state: FSMContext):
    ...
    await Yuk_reys_buxoro.soat.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_buxoro.soat)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )", reply_markup=reys_ortgaa)
    await Yuk_reys_buxoro.kuni.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_buxoro.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\n"
...
@dp.callback_query_handler(state=Yuk_reys_buxoro.soat)
async def reys_soat(call: CallbackQuery, state: FSMContext):
    ...
    await Yuk_reys_buxoro.phone.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_buxoro.phone)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ", reply_markup=time)
    await Yuk_reys_buxoro.soat.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_buxoro.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'], state=Yuk_reys_buxoro.phone)
async def reys_loc(message: Message, state: FSMContext):
    ...
    await Yuk_reys_buxoro.qanday_moshina.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_buxoro.qanday_moshina)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Telefon raqamingizni kiriting ..", reply_markup=phone_number)
    await call.message.answer("Mana shu raqamni ishlatayotgan bo'lsangiz\n"
                              "Kontakt yuborish ni bosing", reply_markup=reys_ortgaa)
...
@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_buxoro.qanday_moshina)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Yuk_reys_buxoro.qanday_moshina)
async def reys_qanday_avto(message: Message, state: FSMContext):
    ...
    await Yuk_reys_buxoro.tasdiqlash.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_buxoro.tasdiqlash)
async def reys_ortga(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Mashinangiz qanday ? ")
    await Yuk_reys_buxoro.qanday_moshina.set()


@dp.callback_query_handler(text='yesss', state=Yuk_reys_buxoro.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Yuk_reys_buxoro.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(text='add_information', state=Yuk_reys_buxoro.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'l haqqi uchun qancha olasiz ? :   ")
    await Yuk_reys_buxoro.xa_yoq.set()


@dp.message_handler(state=Yuk_reys_buxoro.xa_yoq)
async def kisi(message: Message, state: FSMContext):
    ...
    await Yuk_reys_buxoro.pochta_olasizmi.set()


@dp.message_handler(state=Yuk_reys_buxoro.pochta_olasizmi)
async def reys_pochta_olasizmi(message: Message, state: FSMContext):
    ...
    await Yuk_reys_buxoro.yuk_olasizmi.set()


@dp.message_handler(state=Yuk_reys_buxoro.yuk_olasizmi)
async def yuk_olasizmi_reys(message: Message, state: FSMContext):
    ...
    await Yuk_reys_buxoro.jami_odam.set()


@dp.message_handler(state=Yuk_reys_buxoro.jami_odam)
async def jami_odam(message: Message, state: FSMContext):
    ...
    await Yuk_reys_buxoro.odam.set()


@dp.message_handler(state=Yuk_reys_buxoro.odam)
async def reys_odam(message: Message, state: FSMContext):
    ...
    await Yuk_reys_buxoro.end.set()


@dp.callback_query_handler(text='Confirm', state=Yuk_reys_buxoro.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Yuk_reys_buxoro.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
