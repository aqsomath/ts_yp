...
from states.yuk_reys_states import Yuk_reys_qashqadaryo


@dp.callback_query_handler(taxi_yuk_callback.filter(item_name='january'))
async def reys(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Yukni qaysi tumandan olasiz ? ",reply_markup=qashqadaryo_yol)
    await Yuk_reys_qashqadaryo.tuman.set()
@dp.callback_query_handler(text='ortga',state=Yuk_reys_qashqadaryo.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Yuk_reys_qashqadaryo.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Yuk_reys_qashqadaryo.tuman)
async def reys_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_reys_qashqadaryo.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Yuk_reys_qashqadaryo.viloyatga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan ketasiz ? ", reply_markup=qashqadaryo_yol)#############
    await Yuk_reys_qashqadaryo.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_reys_qashqadaryo.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_reys_qashqadaryo.viloyatga)
async def reys_viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_reys_qashqadaryo.tumaniga.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_qashqadaryo.tumaniga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_reys_qashqadaryo.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_qashqadaryo.tumaniga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_reys_qashqadaryo.tumaniga)
async def reys_tumaniga(call: CallbackQuery, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.kuni.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_qashqadaryo.kuni)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga yuk olib borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_reys_qashqadaryo.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_qashqadaryo.kuni)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Yuk_reys_qashqadaryo.kuni)
async def reys_kuni(message: Message, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.soat.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_qashqadaryo.soat)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )", reply_markup=reys_ortgaa)
    await Yuk_reys_qashqadaryo.kuni.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_qashqadaryo.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\n"
...
@dp.callback_query_handler(state=Yuk_reys_qashqadaryo.soat)
async def reys_soat(call: CallbackQuery, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.phone.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_qashqadaryo.phone)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ", reply_markup=time)
    await Yuk_reys_qashqadaryo.soat.set()


@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_qashqadaryo.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'], state=Yuk_reys_qashqadaryo.phone)
async def reys_loc(message: Message, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.qanday_moshina.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_qashqadaryo.qanday_moshina)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Telefon raqamingizni kiriting ..", reply_markup=phone_number)
    await call.message.answer("Mana shu raqamni ishlatayotgan bo'lsangiz\n"
                              "Kontakt yuborish ni bosing", reply_markup=reys_ortgaa)
...
@dp.callback_query_handler(text_contains='atmen', state=Yuk_reys_qashqadaryo.qanday_moshina)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Yuk_reys_qashqadaryo.qanday_moshina)
async def reys_qanday_avto(message: Message, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.tasdiqlash.set()


@dp.callback_query_handler(text='ortga', state=Yuk_reys_qashqadaryo.tasdiqlash)
async def reys_ortga(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Mashinangiz qanday ? ")
    await Yuk_reys_qashqadaryo.qanday_moshina.set()


@dp.callback_query_handler(text='yesss', state=Yuk_reys_qashqadaryo.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Yuk_reys_qashqadaryo.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(text='add_information', state=Yuk_reys_qashqadaryo.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'l haqqi uchun qancha olasiz ? :   ")
    await Yuk_reys_qashqadaryo.xa_yoq.set()


@dp.message_handler(state=Yuk_reys_qashqadaryo.xa_yoq)
async def kisi(message: Message, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.pochta_olasizmi.set()


@dp.message_handler(state=Yuk_reys_qashqadaryo.pochta_olasizmi)
async def reys_pochta_olasizmi(message: Message, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.yuk_olasizmi.set()


@dp.message_handler(state=Yuk_reys_qashqadaryo.yuk_olasizmi)
async def yuk_olasizmi_reys(message: Message, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.jami_odam.set()


@dp.message_handler(state=Yuk_reys_qashqadaryo.jami_odam)
async def jami_odam(message: Message, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.odam.set()


@dp.message_handler(state=Yuk_reys_qashqadaryo.odam)
async def reys_odam(message: Message, state: FSMContext):
    ...
    await Yuk_reys_qashqadaryo.end.set()


@dp.callback_query_handler(text='Confirm', state=Yuk_reys_qashqadaryo.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Yuk_reys_qashqadaryo.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
