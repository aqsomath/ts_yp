...
from states.haydovchi_reys_states import Reys_jizzax
...
@dp.callback_query_handler(taxi_reys_callback.filter(item_name='kvadrat'))
async def reys(call:CallbackQuery,state:FSMContext):
    ...
    await Reys_jizzax.tuman.set()
@dp.callback_query_handler(text='ortga',state=Reys_jizzax.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Reys_jizzax.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Reys_jizzax.tuman)
async def reys_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Reys_jizzax.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Reys_jizzax.viloyatga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan ketasiz ? ", reply_markup=jizzax_yol)#############
    await Reys_jizzax.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_jizzax.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Reys_jizzax.viloyatga)
async def reys_viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Reys_jizzax.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Reys_jizzax.tumaniga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz ? ", reply_markup=viloyatlar_yol)
    await Reys_jizzax.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_jizzax.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Reys_jizzax.tumaniga)
async def reys_tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Reys_jizzax.kuni.set()
@dp.callback_query_handler(text='ortga',state=Reys_jizzax.kuni)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Reys_jizzax.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_jizzax.kuni)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Reys_jizzax.kuni)
async def reys_kuni(message:Message,state:FSMContext):
    ...
    await Reys_jizzax.soat.set()
@dp.callback_query_handler(text='ortga',state=Reys_jizzax.soat)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )",reply_markup=reys_ortgaa)
    await Reys_jizzax.kuni.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_jizzax.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Reys_jizzax.soat)
async def reys_soat(call:CallbackQuery,state:FSMContext):
    ...
    await Reys_jizzax.phone.set()
@dp.callback_query_handler(text='ortga',state=Reys_jizzax.phone)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ",reply_markup=time)
    await Reys_jizzax.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_jizzax.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact','text'],state=Reys_jizzax.phone)
async def reys_loc(message:Message,state:FSMContext):
    ...
    await Reys_jizzax.qanday_moshina.set()
@dp.callback_query_handler(text='ortga',state=Reys_jizzax.qanday_moshina)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Telefon raqamingizni kiriting ..", reply_markup=phone_number)
    await call.message.answer("Mana shu raqamni ishlatayotgan bo'lsangiz\n"
                              "Kontakt yuborish ni bosing", reply_markup=reys_ortgaa)
...
@dp.callback_query_handler(text_contains='atmen',state=Reys_jizzax.qanday_moshina)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.message_handler(state=Reys_jizzax.qanday_moshina)
async def reys_qanday_avto(message:Message,state:FSMContext):
    ...
    await Reys_jizzax.tasdiqlash.set()
@dp.callback_query_handler(text='ortga',state=Reys_jizzax.tasdiqlash)
async def reys_ortga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Mashinangiz qanday ? ")
    await Reys_jizzax.qanday_moshina.set()

@dp.callback_query_handler(text='yesss', state=Reys_jizzax.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Reys_jizzax.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='add_information', state=Reys_jizzax.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Yo'l haqqi uchun qancha olasiz ? :   ")
    await Reys_jizzax.xa_yoq.set()
@dp.message_handler(state=Reys_jizzax.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Reys_jizzax.pochta_olasizmi.set()
@dp.message_handler(state=Reys_jizzax.pochta_olasizmi)
async def reys_pochta_olasizmi(message:Message, state:FSMContext):
    ...
    await Reys_jizzax.yuk_olasizmi.set()
@dp.message_handler(state=Reys_jizzax.yuk_olasizmi)
async def yuk_olasizmi_reys(message:Message,state:FSMContext):
    ...
    await Reys_jizzax.jami_odam.set()
@dp.message_handler(state=Reys_jizzax.jami_odam)
async def jami_odam(message:Message,state:FSMContext):
    ...
    await Reys_jizzax.oldi_orin.set()
@dp.message_handler(state=Reys_jizzax.oldi_orin)
async def oldi_orin(message:Message,state:FSMContext):
    ...
    await Reys_jizzax.odam.set()
@dp.message_handler(state=Reys_jizzax.odam)
async def reys_odam(message:Message,state:FSMContext):
    ...
    await Reys_jizzax.end.set()
@dp.callback_query_handler(text='Confirm',state=Reys_jizzax.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Reys_jizzax.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
