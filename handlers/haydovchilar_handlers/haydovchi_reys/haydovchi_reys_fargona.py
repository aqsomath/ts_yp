...
from handlers.haydovchilar_handlers.haydovchi_reys.hardovchi_reys_andijon import messages_drive
from handlers.users.edit_district.fargona_edit import Bagdod,Beshariq,Buvayda,\
    Ozbekiston,Oltiariq,Fargona,Furqat,Yozyovon,Rishton,Dangara,Uchkoprik,Qoshtepa,\
    Quva,Toshloq,Sox
...
from keyboards.inline.haydovchi_reys.tayyor_taxi_keyboards import fargona_tayyor
...
from states.haydovchi_reys_states import Reys_fargona
...
@dp.callback_query_handler(taxi_reys_callback.filter(item_name='detsi'))
async def reys(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan ketasiz ? ",reply_markup=fargona_yol)
    await Reys_fargona.tuman.set()
@dp.callback_query_handler(text='ortga',state=Reys_fargona.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Reys_fargona.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Reys_fargona.tuman)
async def reys_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Reys_fargona.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Reys_fargona.viloyatga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan ketasiz ? ", reply_markup=fargona_yol)
    await Reys_fargona.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_fargona.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Reys_fargona.viloyatga)
async def reys_viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    if data == "kdjhaigdakhdksa":
        ...
        await call.message.answer("Qaysi tumaniga borasiz", reply_markup=fargona_tayyor)
    ...
    await Reys_fargona.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Reys_fargona.tumaniga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz ? ", reply_markup=viloyatlar_yol)
    await Reys_fargona.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_fargona.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Reys_fargona.tumaniga)
async def reys_tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Reys_fargona.kuni.set()
@dp.callback_query_handler(text='ortga',state=Reys_fargona.kuni)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Reys_fargona.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_fargona.kuni)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.message_handler(state=Reys_fargona.kuni)
async def reys_kuni(message:Message,state:FSMContext):
    ...
    await Reys_fargona.soat.set()
@dp.callback_query_handler(text='ortga',state=Reys_fargona.soat)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )",reply_markup=reys_ortgaa)
    await Reys_fargona.kuni.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_fargona.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Reys_fargona.soat)
async def reys_soat(call:CallbackQuery,state:FSMContext):
    ...
    await Reys_fargona.phone.set()
@dp.callback_query_handler(text='ortga',state=Reys_fargona.phone)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ",reply_markup=time)
    await Reys_fargona.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Reys_fargona.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.message_handler(content_types=['contact','text'],state=Reys_fargona.phone)
async def reys_loc(message:Message,state:FSMContext):
    ...
    await Reys_fargona.qanday_moshina.set()
@dp.callback_query_handler(text='ortga',state=Reys_fargona.qanday_moshina)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Telefon raqamingizni kiriting ..", reply_markup=phone_number)
    await call.message.answer("Mana shu raqamni ishlatayotgan bo'lsangiz\n"
                              "Kontakt yuborish ni bosing", reply_markup=reys_ortgaa)
@dp.callback_query_handler(text_contains='atmen',state=Reys_fargona.qanday_moshina)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Reys_fargona.qanday_moshina)
async def reys_qanday_avto(message:Message,state:FSMContext):
    ...
    await Reys_fargona.tasdiqlash.set()
@dp.callback_query_handler(text='ortga',state=Reys_fargona.tasdiqlash)
async def reys_ortga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Mashinangiz qanday ? ")
    await Reys_fargona.qanday_moshina.set()

@dp.callback_query_handler(text='yesss', state=Reys_fargona.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Reys_fargona.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='add_information', state=Reys_fargona.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Yo'l haqqi uchun qancha olasiz ? :   ")
    await Reys_fargona.xa_yoq.set()
@dp.message_handler(state=Reys_fargona.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Reys_fargona.pochta_olasizmi.set()
@dp.message_handler(state=Reys_fargona.pochta_olasizmi)
async def reys_pochta_olasizmi(message:Message, state:FSMContext):
    ...
    await Reys_fargona.yuk_olasizmi.set()
@dp.message_handler(state=Reys_fargona.yuk_olasizmi)
async def yuk_olasizmi_reys(message:Message,state:FSMContext):
    ...
    await Reys_fargona.jami_odam.set()
@dp.message_handler(state=Reys_fargona.jami_odam)
async def jami_odam(message:Message,state:FSMContext):
    ...
    await Reys_fargona.oldi_orin.set()
@dp.message_handler(state=Reys_fargona.oldi_orin)
async def oldi_orin(message:Message,state:FSMContext):
    ...
    await Reys_fargona.odam.set()
@dp.message_handler(state=Reys_fargona.odam)
async def reys_odam(message:Message,state:FSMContext):
    ...
    await Reys_fargona.end.set()
@dp.callback_query_handler(text='Confirm',state=Reys_fargona.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Reys_fargona.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
