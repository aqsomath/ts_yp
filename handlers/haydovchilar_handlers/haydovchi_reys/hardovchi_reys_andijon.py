...
from keyboards.inline.yolovchi.kirish import tasdiq_oxir, umumiy_menu_1, kirish, umumiy_menu
...
@dp.callback_query_handler(state=Reys_andijon.qoshimcha_tumanlarga)
...
@dp.callback_query_handler(text="ortga", state=Reys_andijon.qoshimcha_tumanlarga)
...
async def yuk_olasizmi_reysass(call: CallbackQuery, state: FSMContext):
    ...
    markup.insert(aiogram.types.InlineKeyboardButton(text='Bosh menu', callback_data='boshmenu'))
    markup.insert(aiogram.types.InlineKeyboardButton(text='Keyingisi', callback_data='Keyingisi'))
    await call.message.answer("Nechta yo'lovchi olasiz ? ", reply_markup=markup)
    await call.message.delete()
