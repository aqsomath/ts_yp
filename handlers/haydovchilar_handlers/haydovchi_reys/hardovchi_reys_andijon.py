...
@dp.callback_query_handler(taxi_reys_callback.filter(item_name="santi"))
...
@dp.callback_query_handler(text='ortga',state=Reys_andijon.viloyatga)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan ketasiz ? ", reply_markup=andijon_yol)#############
    await Reys_andijon.tuman.set()
...
@dp.callback_query_handler(text='ortga',state=Reys_andijon.phone)
async def andi_jon(call:CallbackQuery,state:FSMContext):
...
async def reys_loc(message:Message,state:FSMContext):
    ...
    await Reys_andijon.qanday_moshina.set()
@dp.callback_query_handler(text='ortga',state=Reys_andijon.qanday_moshina)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Telefon raqamingizni kiriting ..", reply_markup=phone_number)
    await call.message.answer("Mana shu raqamni ishlatayotgan bo'lsangiz\n"
                              "Kontakt yuborish ni bosing", reply_markup=reys_ortgaa)
...
@dp.message_handler(state=Reys_andijon.xa_yoq)
...
@dp.message_handler(state=Reys_andijon.pochta_olasizmi)
...
@dp.message_handler(state=Reys_andijon.yuk_olasizmi)
...
@dp.message_handler(state=Reys_andijon.jami_odam)
async def jami_odam(message:Message,state:FSMContext):
    ...
    await Reys_andijon.oldi_orin.set()
@dp.message_handler(state=Reys_andijon.oldi_orin)
async def oldi_orin(message:Message,state:FSMContext):
    ...
    await Reys_andijon.odam.set()
@dp.message_handler(state=Reys_andijon.odam)
