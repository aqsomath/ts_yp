...
@dp.callback_query_handler(taxi_sayohat_callback.filter(item_name='alif'))
async def reys(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Sayohatchilarni qaysi tumandan olasiz ? ", reply_markup=andijon_yol)
...
@dp.callback_query_handler(text='ortga', state=Hay_say_andijon.viloyatga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Sayohatchilarni qaysi tumandan olasiz ? ", reply_markup=andijon_yol)
    await Hay_say_andijon.tuman.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_andijon.viloyatga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.message_handler(state=Hay_say_andijon.viloyatga)
...
@dp.message_handler(state=Hay_say_andijon.kuni)
...
async def y_n(call: CallbackQuery, state: FSMContext):
    ...
    print("Qo'shildi")
    order = await db.select_tayyor_sayohatchi_mashina()
    print(order)
...
@dp.message_handler(state=Hay_say_andijon.xa_yoq)
...
@dp.message_handler(state=Hay_say_andijon.pochta_olasizmi)
async def reys_pochta_olasizmi(message: Message, state: FSMContext):
    ...
    await Hay_say_andijon.odam.set()



@dp.message_handler(state=Hay_say_andijon.odam)
