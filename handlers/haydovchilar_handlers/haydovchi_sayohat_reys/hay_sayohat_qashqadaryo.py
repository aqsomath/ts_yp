...
from states.haydovchi_sayohat_reys_states import Hay_say_qashqadaryo


@dp.callback_query_handler(taxi_sayohat_callback.filter(item_name='ayn'))
async def reys(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Sayohatchilarni qaysi tumandan olasiz ? ", reply_markup=qashqadaryo_yol)
    await Hay_say_qashqadaryo.tuman.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_qashqadaryo.tuman)
...
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen', state=Hay_say_qashqadaryo.tuman)
...
@dp.callback_query_handler(state=Hay_say_qashqadaryo.tuman)
async def reys_tuman(call: CallbackQuery, state: FSMContext):
    ...
    await Hay_say_qashqadaryo.viloyatga.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_qashqadaryo.viloyatga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Sayohatchilarni qaysi tumandan olasiz ? ", reply_markup=qashqadaryo_yol)
    await Hay_say_qashqadaryo.tuman.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_qashqadaryo.viloyatga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.message_handler(state=Hay_say_qashqadaryo.viloyatga)
async def reys_tumaniga(message: Message, state: FSMContext):
    ...
    await Hay_say_qashqadaryo.kuni.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_qashqadaryo.kuni)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatlarga borasiz ? ( Yozma shaklda kiriting ) ",reply_markup=reys_ortgaa)
    await Hay_say_qashqadaryo.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_qashqadaryo.kuni)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Hay_say_qashqadaryo.kuni)
async def reys_kuni(message: Message, state: FSMContext):
    ...
    await Hay_say_qashqadaryo.soat.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_qashqadaryo.soat)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )", reply_markup=reys_ortgaa)
    await Hay_say_qashqadaryo.kuni.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_qashqadaryo.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\n"
...
@dp.callback_query_handler(state=Hay_say_qashqadaryo.soat)
async def reys_soat(call: CallbackQuery, state: FSMContext):
    ...
    await Hay_say_qashqadaryo.phone.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_qashqadaryo.phone)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ", reply_markup=time)
    await Hay_say_qashqadaryo.soat.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_qashqadaryo.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'], state=Hay_say_qashqadaryo.phone)
async def reys_loc(message: Message, state: FSMContext):
    ...
    await Hay_say_qashqadaryo.tasdiqlash.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_qashqadaryo.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Hay_say_qashqadaryo.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Hay_say_qashqadaryo.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(text='add_information', state=Hay_say_qashqadaryo.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Mahsinangiz qanday ? :   ")
    await Hay_say_qashqadaryo.xa_yoq.set()


@dp.message_handler(state=Hay_say_qashqadaryo.xa_yoq)
async def kisi(message: Message, state: FSMContext):
    ...
    await Hay_say_qashqadaryo.pochta_olasizmi.set()


@dp.message_handler(state=Hay_say_qashqadaryo.pochta_olasizmi)
async def reys_pochta_olasizmi(message: Message, state: FSMContext):
    ...
    await Hay_say_qashqadaryo.odam.set()



@dp.message_handler(state=Hay_say_qashqadaryo.odam)
async def reys_odam(message: Message, state: FSMContext):
    ...
    await Hay_say_qashqadaryo.end.set()


@dp.callback_query_handler(text='Confirm', state=Hay_say_qashqadaryo.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Hay_say_qashqadaryo.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
