...
from states.haydovchi_sayohat_reys_states import Hay_say_namangan


@dp.callback_query_handler(taxi_sayohat_callback.filter(item_name='ba'))
async def reys(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Sayohatchilarni qaysi tumandan olasiz ? ", reply_markup=namangan_yol)
    await Hay_say_namangan.tuman.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_namangan.tuman)
...
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen', state=Hay_say_namangan.tuman)
...
@dp.callback_query_handler(state=Hay_say_namangan.tuman)
async def reys_tuman(call: CallbackQuery, state: FSMContext):
    ...
    await Hay_say_namangan.viloyatga.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_namangan.viloyatga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Sayohatchilarni qaysi tumandan olasiz ? ", reply_markup=namangan_yol)
    await Hay_say_namangan.tuman.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_namangan.viloyatga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.message_handler(state=Hay_say_namangan.viloyatga)
async def reys_tumaniga(message: Message, state: FSMContext):
    ...
    await Hay_say_namangan.kuni.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_namangan.kuni)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatlarga borasiz ? ( Yozma shaklda kiriting ) ",reply_markup=reys_ortgaa)
    await Hay_say_namangan.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_namangan.kuni)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Hay_say_namangan.kuni)
async def reys_kuni(message: Message, state: FSMContext):
    ...
    await Hay_say_namangan.soat.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_namangan.soat)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )", reply_markup=reys_ortgaa)
    await Hay_say_namangan.kuni.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_namangan.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\n"
...
@dp.callback_query_handler(state=Hay_say_namangan.soat)
async def reys_soat(call: CallbackQuery, state: FSMContext):
    ...
    await Hay_say_namangan.phone.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_namangan.phone)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ", reply_markup=time)
    await Hay_say_namangan.soat.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_namangan.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'], state=Hay_say_namangan.phone)
async def reys_loc(message: Message, state: FSMContext):
    ...
    await Hay_say_namangan.tasdiqlash.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_namangan.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Hay_say_namangan.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Hay_say_namangan.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(text='add_information', state=Hay_say_namangan.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Mahsinangiz qanday ? :   ")
    await Hay_say_namangan.xa_yoq.set()


@dp.message_handler(state=Hay_say_namangan.xa_yoq)
async def kisi(message: Message, state: FSMContext):
    ...
    await Hay_say_namangan.pochta_olasizmi.set()


@dp.message_handler(state=Hay_say_namangan.pochta_olasizmi)
async def reys_pochta_olasizmi(message: Message, state: FSMContext):
    ...
    await Hay_say_namangan.odam.set()



@dp.message_handler(state=Hay_say_namangan.odam)
async def reys_odam(message: Message, state: FSMContext):
    ...
    await Hay_say_namangan.end.set()


@dp.callback_query_handler(text='Confirm', state=Hay_say_namangan.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Hay_say_namangan.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
