...
from states.haydovchi_sayohat_reys_states import Hay_say_buxoro


@dp.callback_query_handler(taxi_sayohat_callback.filter(item_name='sa'))
async def reys(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Sayohatchilarni qaysi tumandan olasiz ? ", reply_markup=buxoro_yol)
    await Hay_say_buxoro.tuman.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_buxoro.tuman)
...
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen', state=Hay_say_buxoro.tuman)
...
@dp.callback_query_handler(state=Hay_say_buxoro.tuman)
async def reys_tuman(call: CallbackQuery, state: FSMContext):
    ...
    await Hay_say_buxoro.viloyatga.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_buxoro.viloyatga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Sayohatchilarni qaysi tumandan olasiz ? ", reply_markup=buxoro_yol)
    await Hay_say_buxoro.tuman.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_buxoro.viloyatga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.message_handler(state=Hay_say_buxoro.viloyatga)
async def reys_tumaniga(message: Message, state: FSMContext):
    ...
    await Hay_say_buxoro.kuni.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_buxoro.kuni)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatlarga borasiz ? ( Yozma shaklda kiriting ) ",reply_markup=reys_ortgaa)
    await Hay_say_buxoro.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_buxoro.kuni)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Hay_say_buxoro.kuni)
async def reys_kuni(message: Message, state: FSMContext):
    ...
    await Hay_say_buxoro.soat.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_buxoro.soat)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )", reply_markup=reys_ortgaa)
    await Hay_say_buxoro.kuni.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_buxoro.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\n"
...
@dp.callback_query_handler(state=Hay_say_buxoro.soat)
async def reys_soat(call: CallbackQuery, state: FSMContext):
    ...
    await Hay_say_buxoro.phone.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_buxoro.phone)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ", reply_markup=time)
    await Hay_say_buxoro.soat.set()


@dp.callback_query_handler(text_contains='atmen', state=Hay_say_buxoro.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'], state=Hay_say_buxoro.phone)
async def reys_loc(message: Message, state: FSMContext):
    ...
    await Hay_say_buxoro.tasdiqlash.set()


@dp.callback_query_handler(text='ortga', state=Hay_say_buxoro.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Hay_say_buxoro.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Hay_say_buxoro.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(text='add_information', state=Hay_say_buxoro.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Mahsinangiz qanday ? :   ")
    await Hay_say_buxoro.xa_yoq.set()


@dp.message_handler(state=Hay_say_buxoro.xa_yoq)
async def kisi(message: Message, state: FSMContext):
    ...
    await Hay_say_buxoro.pochta_olasizmi.set()


@dp.message_handler(state=Hay_say_buxoro.pochta_olasizmi)
async def reys_pochta_olasizmi(message: Message, state: FSMContext):
    ...
    await Hay_say_buxoro.odam.set()



@dp.message_handler(state=Hay_say_buxoro.odam)
async def reys_odam(message: Message, state: FSMContext):
    ...
    await Hay_say_buxoro.end.set()


@dp.callback_query_handler(text='Confirm', state=Hay_say_buxoro.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Hay_say_buxoro.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
