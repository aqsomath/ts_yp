...
@dp.callback_query_handler(state=Reys_pochta_andijon.asosiy)
async def andijon(call: CallbackQuery, state: FSMContext):
            ...
            if call.data=='devit':
                ...
                await Reys_pochta_andijon.tuman.set()
            if call.data=='raz':
            ...
            if call.data=='qoraqalpoq':
                ...
                await call.message.delete()
                await Reys_pochta_andijon.tuman.set()
@dp.callback_query_handler(text='ortga', state=Reys_pochta_andijon.tuman)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    
        await call.message.answer("Qaysi viloyatdan pochta kerak ? ", reply_markup=tax_pochta_vil)
        await call.message.delete()

        await Reys_pochta_andijon.asosiy.set()

#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_andijon.tuman)
...
@dp.callback_query_handler(state=Reys_pochta_andijon.tuman)
async def reys_tuman(call: CallbackQuery, state: FSMContext):
        ...
        await call.message.delete()
        await Reys_pochta_andijon.viloyatga.set()


@dp.callback_query_handler(text='homeback', state=Reys_pochta_andijon.viloyatga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_andijon.viloyatga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    user = await db.select_user(telegram_id=call.from_user.id)
    if user is not None:
        if user[5] == True:
...
@dp.callback_query_handler(state=Reys_pochta_andijon.viloyatga)
async def reys_viloyatga(call: CallbackQuery, state: FSMContext):
        ...
        await call.message.delete()
        await Reys_pochta_andijon.tumaniga.set()
@dp.callback_query_handler(text='ortga', state=Reys_pochta_andijon.tumaniga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_andijon.tumaniga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    user = await db.select_user(telegram_id=call.from_user.id)
    if user is not None:
        if user[5] == True:
...
@dp.callback_query_handler(state=Reys_pochta_andijon.tumaniga)
async def reys_tumaniga(call: CallbackQuery, state: FSMContext):
        ...
        await call.message.delete()
        await Reys_pochta_andijon.kuni.set()


@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.kuni)
...
@dp.callback_query_handler(text="atmen", state=Reys_pochta_andijon.kuni)
async def taqas(call: CallbackQuery, state: FSMContext):
    user = await db.select_user(telegram_id=call.from_user.id)
    if user is not None:
        if user[5] == True:
...
@dp.callback_query_handler(text='Qoldakiritish', state=Reys_pochta_andijon.kuni)
async def qolda_yozing(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await Reys_pochta_andijon.oyini_kiritsh.set()


@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.oyini_kiritsh)
...
@dp.callback_query_handler(text="Ortga", state=Reys_pochta_andijon.oyini_kiritsh)
...
@dp.callback_query_handler(state=Reys_pochta_andijon.oyini_kiritsh)
async def oyi(call: CallbackQuery, state: FSMContext):
    ...
    if int(call.data) < datetime.datetime.now().month:
        ...
        await call.message.delete()
        await Reys_pochta_andijon.kunini_kiritsh.set()


@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.kunini_kiritsh)
async def bosh(call: CallbackQuery, state: FSMContext):
    user = await db.select_user(telegram_id=call.from_user.id)
    if user is not None:
        if user[5] == True:
...
@dp.callback_query_handler(text="Ortga", state=Reys_pochta_andijon.kunini_kiritsh)
async def qayyt(call: CallbackQuery, state: FSMContext):
    markup = InlineKeyboardMarkup(row_width=6)
    markup.insert(InlineKeyboardButton(text="Yanvar", callback_data="1"))
    markup.insert(InlineKeyboardButton(text="Fevral", callback_data="2"))
...
@dp.callback_query_handler(state=Reys_pochta_andijon.kunini_kiritsh)
async def kunini(call: CallbackQuery, state: FSMContext):
    ...
    if oy > datetime.datetime.now().month:
        ...
        await call.message.delete()
        await Reys_pochta_andijon.soat.set()
    if oy == datetime.datetime.now().month:
...
@dp.callback_query_handler(text='Bugun', state=Reys_pochta_andijon.kuni)
@dp.callback_query_handler(text='Ertaga', state=Reys_pochta_andijon.kuni)
@dp.callback_query_handler(text='Indinga', state=Reys_pochta_andijon.kuni)
...
@dp.callback_query_handler(text='qaytish', state=Reys_pochta_andijon.aniq_kuni)
...
@dp.callback_query_handler(text='bomenyu', state=Reys_pochta_andijon.aniq_kuni)
...
@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_andijon.kuni)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    user = await db.select_user(telegram_id=call.from_user.id)
    if user is not None:
        if user[5] == True:
...
@dp.callback_query_handler(state=Reys_pochta_andijon.aniq_kuni)
...
@dp.callback_query_handler(text='ortga', state=Reys_pochta_andijon.soat)
async def andi_jon(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_andijon.soat)
async def haydovchidsd(call: CallbackQuery, state: FSMContext):
    user = await db.select_user(telegram_id=call.from_user.id)
    if user is not None:
        if user[5] == True:
            await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?",
...
@dp.callback_query_handler(state=Reys_pochta_andijon.soat)
async def reys_soat(call: CallbackQuery, state: FSMContext):
        ...
        start_time = datetime.datetime(now.year, now.month, now.day, now.hour, now.minute+2, now.second)
        ...
        if time_difference_seconds > 0:
            ...
            await call.message.delete()
            await Reys_pochta_andijon.phone.set()
        else:
...
@dp.callback_query_handler(text='tortga', state=Reys_pochta_andijon.phone)
async def andi_jon(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_andijon.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    user = await db.select_user(telegram_id=call.from_user.id)
    if user is not None:
        if user[5] == True:
...
@dp.message_handler(content_types=['contact', 'text'], state=Reys_pochta_andijon.phone)
async def reys_loc(message: Message, state: FSMContext):
        ...
        if oy is not None:
            m = f"🚕<b>POCHTA AVTO</b>\n<b> 🏢 {viloyat} </b>\n" \
                ...
                f"📆 <b>kuni : {kuni}.{oy}</b>\n" \
                f"⏱ <b>{soat}</b>\n"
            msg = f"🚕<b>POCHTA AVTO</b>\n<b> 🏢 {viloyat} </b>\n" \
                  f"🏤<b> {tuman} dan </b> \n" \
                  f"🏢<b> {viloyatiga} </b>\n" \
                  f"🏪 <b>{tumaniga} ga boruvchi pochta taxi </b>\n" \
        ...
        await Reys_pochta_andijon.tasdiqlash.set()
        await message.delete()

@dp.callback_query_handler(text='ortga', state=Reys_pochta_andijon.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Reys_pochta_andijon.tasdiqlash)
async def sqsdsx(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(text='add_information', state=Reys_pochta_andijon.tasdiqlash)
async def sasassasas(call: CallbackQuery, state: FSMContext):
    ...
    await Reys_pochta_andijon.qoshimcha_tumanlarga.set()


@dp.callback_query_handler(state=Reys_pochta_andijon.qoshimcha_tumanlarga)
...
@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.qoshimcha_tumanlarga)
...
@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.xa_yoq)
...
@dp.callback_query_handler(text="qoldayozish", state=Reys_pochta_andijon.xa_yoq)
async def qlda_yoz(call: CallbackQuery, state: FSMContext):
        ...
        await call.message.delete()
        await Reys_pochta_andijon.qolda_yoz.set()

@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.xa_yoq)
async def ortga_rumanlarga(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qo'shimcha qaysi viloyatlarning qaysi tumaniga borasiz ?", reply_markup=viloyatlar_yol)
    await Reys_pochta_andijon.qoshimcha_tumanlarga.set()
@dp.callback_query_handler(text="Keyingisi", state=Reys_pochta_andijon.xa_yoq)
async def jeieir(call: CallbackQuery, state: FSMContext):
        ...
        await call.message.delete()
        await Reys_pochta_andijon.pochta_olasizmi.set()


@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.qolda_yoz)
...
@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.qolda_yoz)
...
@dp.message_handler(state=Reys_pochta_andijon.qolda_yoz)
...
@dp.callback_query_handler(state=Reys_pochta_andijon.xa_yoq)
...
@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.pochta_olasizmi)
...
@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.pochta_olasizmi)
...
@dp.callback_query_handler(text="Keyingisi", state=Reys_pochta_andijon.pochta_olasizmi)
async def reys_pochta_olaedwewasizmi(call: CallbackQuery, state: FSMContext):
        ...
        await call.message.delete()
        await Reys_pochta_andijon.yuk_olasizmi.set()



@dp.callback_query_handler(state=Reys_pochta_andijon.pochta_olasizmi)
...
@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.yuk_olasizmi)
...
@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.yuk_olasizmi)
...
@dp.callback_query_handler(text="Keyingisi", state=Reys_pochta_andijon.yuk_olasizmi)
async def yuk_olasizmi_reysass(call: CallbackQuery, state: FSMContext):
        ...
        markup.insert(aiogram.types.InlineKeyboardButton(text='Bosh menu', callback_data='boshmenu'))
        markup.insert(aiogram.types.InlineKeyboardButton(text='Keyingisi', callback_data='Keyingisi'))
        await call.message.answer("Nechtagacha pochta olasiz ? ", reply_markup=markup)
        await call.message.delete()
        await Reys_pochta_andijon.jami_odam.set()

@dp.callback_query_handler(state=Reys_pochta_andijon.yuk_olasizmi)
...
@dp.callback_query_handler(text="qoldayozish",state=Reys_pochta_andijon.jami_odam)
async def qolda_kiritish(call:CallbackQuery,state:FSMContext):
    
        await call.message.answer("Nechta pochta olishingizni yozma kiriting .")
        await call.message.delete()
        await Reys_pochta_andijon.qolda_odam_soni.set()

@dp.message_handler(state=Reys_pochta_andijon.qolda_odam_soni)
async def qolda_odam_soni(message:Message,state:FSMContext):
        ...
        if message.text.isdigit()==True:
            ...
            await message.delete()
            await Reys_pochta_andijon.locatsiya.set()
        else:
            await message.answer("Iltimos son kiritib ifodalang. Matn bilan emas !!!")
            await Reys_pochta_andijon.qolda_odam_soni.set()

@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.jami_odam)
...
@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.jami_odam)
...
@dp.callback_query_handler(text="Keyingisi",state=Reys_pochta_andijon.jami_odam)
...
@dp.callback_query_handler(state=Reys_pochta_andijon.jami_odam)
...
@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.locatsiya)
async def bosh_menu(call:CallbackQuery,state:FSMContext):
    ...
    if user is not None:
        if user[5] == True:
            ...
            await call.message.answer(f"Salom, {call.from_user.full_name}!", reply_markup=kirish)
            await state.finish()
@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.locatsiya)
...
@dp.callback_query_handler(text="ruchnoy", state=Reys_pochta_andijon.locatsiya)
async def pol_ruchnoy(call: CallbackQuery, state: FSMContext):
        ...
        await call.message.delete()
        await Reys_pochta_andijon.pul_qol.set()

@dp.message_handler(state=Reys_pochta_andijon.pul_qol)
...
@dp.callback_query_handler(text="Keyingisi", state=Reys_pochta_andijon.locatsiya)
...
@dp.callback_query_handler(state=Reys_pochta_andijon.locatsiya)
...
@dp.callback_query_handler(text="ortga", state=Reys_pochta_andijon.odam_vil)
...
@dp.callback_query_handler(text="boshmenu", state=Reys_pochta_andijon.odam_vil)
...
@dp.callback_query_handler(state=Reys_pochta_andijon.odam_vil)
async def odam_viloyat(call: CallbackQuery, state: FSMContext):
        ...
        if call.data == 'tanladim':
            ...
            await call.message.delete()
            await Reys_pochta_andijon.end.set()
@dp.callback_query_handler(text='boshmenu', state=Reys_pochta_andijon.end)
...
@dp.callback_query_handler(text='qaytish', state=Reys_pochta_andijon.end)
...
@dp.callback_query_handler(text='glavmenu', state=Reys_pochta_andijon.end)
...
@dp.callback_query_handler(text='Confirm', state=Reys_pochta_andijon.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Reys_pochta_andijon.end)
