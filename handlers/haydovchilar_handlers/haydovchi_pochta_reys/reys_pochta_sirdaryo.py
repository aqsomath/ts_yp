...
from states.haydovchi_pochta_states import Reys_pochta_sirdaryo


@dp.callback_query_handler(taxi_pochta_callback.filter(item_name='shest'))
async def reys(call: CallbackQuery, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.tuman.set()


@dp.callback_query_handler(text='ortga', state=Reys_pochta_sirdaryo.tuman)
...
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_sirdaryo.tuman)
...
@dp.callback_query_handler(state=Reys_pochta_sirdaryo.tuman)
async def reys_tuman(call: CallbackQuery, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.viloyatga.set()


@dp.callback_query_handler(text='ortga', state=Reys_pochta_sirdaryo.viloyatga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi tumanidan ketasiz ? ", reply_markup=sirdaryo_yol)  #############
    await Reys_pochta_sirdaryo.tuman.set()


@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_sirdaryo.viloyatga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Reys_pochta_sirdaryo.viloyatga)
async def reys_viloyatga(call: CallbackQuery, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.tumaniga.set()


@dp.callback_query_handler(text='ortga', state=Reys_pochta_sirdaryo.tumaniga)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Reys_pochta_sirdaryo.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_sirdaryo.tumaniga)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Reys_pochta_sirdaryo.tumaniga)
async def reys_tumaniga(call: CallbackQuery, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.kuni.set()


@dp.callback_query_handler(text='ortga', state=Reys_pochta_sirdaryo.kuni)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga yuk olib borasiz ? ", reply_markup=viloyatlar_yol_x)
    await Reys_pochta_sirdaryo.viloyatga.set()


@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_sirdaryo.kuni)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(state=Reys_pochta_sirdaryo.kuni)
async def reys_kuni(message: Message, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.soat.set()


@dp.callback_query_handler(text='ortga', state=Reys_pochta_sirdaryo.soat)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Yo'lga chiqadigan sanangizni yozing. ( M-n : 11.07.2023 )", reply_markup=reys_ortgaa)
    await Reys_pochta_sirdaryo.kuni.set()


@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_sirdaryo.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\n"
...
@dp.callback_query_handler(state=Reys_pochta_sirdaryo.soat)
async def reys_soat(call: CallbackQuery, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.phone.set()


@dp.callback_query_handler(text='ortga', state=Reys_pochta_sirdaryo.phone)
async def andi_jon(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqasiz ? ", reply_markup=time)
    await Reys_pochta_sirdaryo.soat.set()


@dp.callback_query_handler(text_contains='atmen', state=Reys_pochta_sirdaryo.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'], state=Reys_pochta_sirdaryo.phone)
async def reys_loc(message: Message, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.tasdiqlash.set()


@dp.callback_query_handler(text='ortga', state=Reys_pochta_sirdaryo.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Reys_pochta_sirdaryo.tasdiqlash)
...
@dp.callback_query_handler(text='nott', state=Reys_pochta_sirdaryo.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(text='add_information', state=Reys_pochta_sirdaryo.tasdiqlash)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Mahsinangiz qanday ? :   ")
    await Reys_pochta_sirdaryo.xa_yoq.set()


@dp.message_handler(state=Reys_pochta_sirdaryo.xa_yoq)
async def kisi(message: Message, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.pochta_olasizmi.set()


@dp.message_handler(state=Reys_pochta_sirdaryo.pochta_olasizmi)
async def reys_pochta_olasizmi(message: Message, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.yuk_olasizmi.set()


@dp.message_handler(state=Reys_pochta_sirdaryo.yuk_olasizmi)
async def yuk_olasizmi_reys(message: Message, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.jami_odam.set()


@dp.message_handler(state=Reys_pochta_sirdaryo.jami_odam)
async def jami_odam(message: Message, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.odam.set()


@dp.message_handler(state=Reys_pochta_sirdaryo.odam)
async def reys_odam(message: Message, state: FSMContext):
    ...
    await Reys_pochta_sirdaryo.end.set()


@dp.callback_query_handler(text='Confirm', state=Reys_pochta_sirdaryo.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Reys_pochta_sirdaryo.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
