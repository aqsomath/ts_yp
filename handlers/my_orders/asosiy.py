...
async def call_qur(call:CallbackQuery,state:FSMContext):
    ...
    markup.add(aiogram.types.InlineKeyboardButton(text="Yo'ovchi uchun buyurtmalar", callback_data="jsegukwsgfsgjkfsdgs"))
    markup.add(aiogram.types.InlineKeyboardButton(text="Yuk olish uchun buyurtmalar", callback_data="yukavtochaqirish"))
    markup.add(aiogram.types.InlineKeyboardButton(text="Sayohatchi olish uchun buyurtmalar", callback_data="sayohattaxichaqirish"))
    markup.add(aiogram.types.InlineKeyboardButton(text="Pochta yuborish buyurtmalari", callback_data="pochtayuborishbuyurtmalari"))
...
async def taxi_buyurtmalarim(call:CallbackQuery,state:FSMContext):
    ...
    if len(orders)==0:
        ...
        await state.update_data({"taxi_list":orders_yuk_avto_list})
