...
@dp.callback_query_handler(yuk_callback.filter(item_name='9999'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan yuborasiz ? ",reply_markup=xorazm_yol)
...
@dp.callback_query_handler(text='ortga',state=Yuk_xorazm.viloyatga)
async def ztumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan yuborasiz ? ", reply_markup=xorazm_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Yuk_xorazm.tuman.set()
...
@dp.callback_query_handler(text='ortga',state=Yuk_xorazm.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Yuk_xorazm.soat.set()
...
@dp.callback_query_handler(text='ortga',state=Yuk_xorazm.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Yuk_xorazm.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_xorazm.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Yuk_xorazm.locatsiya)
...
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_xorazm.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_xorazm.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
        ...
        if i[2] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='s45d4s545s4d998989'))
...
@dp.callback_query_handler(text="s45d4s545s4d998989")
...
async def y_n(call:CallbackQuery, state:FSMContext):
    ...
    await Yuk_xorazm.xa_yoq.set()
@dp.callback_query_handler(state=Yuk_xorazm.xa_yoq)
async def kisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_xorazm.turi.set()
@dp.message_handler(state=Yuk_xorazm.turi)
async def yuk_turi(message:Message,state:FSMContext):
    ...
    await Yuk_xorazm.narx.set()
@dp.message_handler(state=Yuk_xorazm.narx)
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='sa5as5dd5f5ff5f55f6w6w6d6de'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
...
@dp.callback_query_handler(text="sa5as5dd5f5ff5f55f6w6w6d6de")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
