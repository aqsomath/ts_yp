...
from states.yuk_states import Yuk_surxondaryo
...
@dp.callback_query_handler(yuk_callback.filter(item_name='7777'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan yuborasiz ? ",reply_markup=surxondaryo_yol)
    await Yuk_surxondaryo.tuman.set()
# 1 - ORTGA
@dp.callback_query_handler(text='ortga',state=Yuk_surxondaryo.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Yukni qaysi viloyatdan yuborasiz ?", reply_markup=yuk_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Yuk_surxondaryo.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Yuk_surxondaryo.tuman)
async def tumanx(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_surxondaryo.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Yuk_surxondaryo.viloyatga)
async def ztumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan yuborasiz ? ", reply_markup=surxondaryo_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Yuk_surxondaryo.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_surxondaryo.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_surxondaryo.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_surxondaryo.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Yuk_surxondaryo.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_surxondaryo.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_surxondaryo.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_surxondaryo.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Yuk_surxondaryo.soat.set()
@dp.callback_query_handler(text='ortga',state=Yuk_surxondaryo.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_surxondaryo.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_surxondaryo.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_surxondaryo.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_surxondaryo.phone.set()
@dp.callback_query_handler(text='ortga',state=Yuk_surxondaryo.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Yuk_surxondaryo.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_surxondaryo.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Yuk_surxondaryo.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Yuk_surxondaryo.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Yuk_surxondaryo.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Yuk_surxondaryo.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_surxondaryo.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Yuk_surxondaryo.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_surxondaryo.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Yuk_surxondaryo.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Yuk_surxondaryo.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_surxondaryo.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_surxondaryo.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Yuk_surxondaryo.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Yuk_surxondaryo.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
        ...
        if i[2] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='WEDWASAA65AS6565'))
...
@dp.callback_query_handler(text="WEDWASAA65AS6565")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Yuk_surxondaryo.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Qanday yuk mashinasi kerak ? ",reply_markup= qanaqa_avto)
    await Yuk_surxondaryo.xa_yoq.set()
@dp.callback_query_handler(state=Yuk_surxondaryo.xa_yoq)
async def kisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_surxondaryo.turi.set()
@dp.message_handler(state=Yuk_surxondaryo.turi)
async def yuk_turi(message:Message,state:FSMContext):
    ...
    await Yuk_surxondaryo.narx.set()
@dp.message_handler(state=Yuk_surxondaryo.narx)
async def yuk_narx(message:Message,state:FSMContext):
    ...
    await Yuk_surxondaryo.end.set()
@dp.callback_query_handler(text='Confirm',state=Yuk_surxondaryo.end)
...
@dp.callback_query_handler(text="EWEDD545D4F5D4")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Yuk_surxondaryo.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()