...
from states.yuk_states import Yuk_fargona
...
@dp.callback_query_handler(yuk_callback.filter(item_name='333'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan yuborasiz ? ",reply_markup=fargona_yol)
    await Yuk_fargona.tuman.set()
# 1 - ORTGA
@dp.callback_query_handler(text='ortga',state=Yuk_fargona.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Yukni qaysi viloyatdan yuborasiz ?", reply_markup=yuk_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Yuk_fargona.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Yuk_fargona.tuman)
async def tumanx(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_fargona.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Yuk_fargona.viloyatga)
async def ztumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan yuborasiz ? ", reply_markup=fargona_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Yuk_fargona.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_fargona.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_fargona.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_fargona.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Yuk_fargona.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_fargona.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_fargona.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_fargona.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Yuk_fargona.soat.set()
@dp.callback_query_handler(text='ortga',state=Yuk_fargona.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_fargona.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_fargona.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_fargona.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_fargona.phone.set()
@dp.callback_query_handler(text='ortga',state=Yuk_fargona.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Yuk_fargona.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_fargona.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Yuk_fargona.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Yuk_fargona.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Yuk_fargona.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Yuk_fargona.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_fargona.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Yuk_fargona.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_fargona.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Yuk_fargona.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Yuk_fargona.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_fargona.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_fargona.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Yuk_fargona.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Yuk_fargona.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
        ...
        if i[2] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='8d86s6s65d4s6d54f'))
...
@dp.callback_query_handler(text="8d86s6s65d4s6d54f")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Yuk_fargona.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Qanday yuk mashinasi kerak ? ",reply_markup= qanaqa_avto)
    await Yuk_fargona.xa_yoq.set()
@dp.callback_query_handler(state=Yuk_fargona.xa_yoq)
async def kisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_fargona.turi.set()
@dp.message_handler(state=Yuk_fargona.turi)
async def yuk_turi(message:Message,state:FSMContext):
    ...
    await Yuk_fargona.narx.set()
@dp.message_handler(state=Yuk_fargona.narx)
async def yuk_narx(message:Message,state:FSMContext):
    ...
    await Yuk_fargona.end.set()
@dp.callback_query_handler(text='Confirm',state=Yuk_fargona.end)
...
@dp.callback_query_handler(text="wowooeooeiuru")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)

@dp.callback_query_handler(text='UnConfirm', state=Yuk_fargona.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()