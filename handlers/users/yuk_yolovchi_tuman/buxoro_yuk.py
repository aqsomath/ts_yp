...
from states.yuk_states import Yuk_buxoro
...
@dp.callback_query_handler(yuk_callback.filter(item_name='4444'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan yuborasiz ? ",reply_markup=buxoro_yol)
    await Yuk_buxoro.tuman.set()
# 1 - ORTGA
@dp.callback_query_handler(text='ortga',state=Yuk_buxoro.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Yukni qaysi viloyatdan yuborasiz ?", reply_markup=yuk_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Yuk_buxoro.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Yuk_buxoro.tuman)
async def tumanx(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_buxoro.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Yuk_buxoro.viloyatga)
async def ztumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan yuborasiz ? ", reply_markup=buxoro_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Yuk_buxoro.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_buxoro.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_buxoro.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_buxoro.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Yuk_buxoro.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_buxoro.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_buxoro.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_buxoro.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Yuk_buxoro.soat.set()
@dp.callback_query_handler(text='ortga',state=Yuk_buxoro.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_buxoro.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_buxoro.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_buxoro.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_buxoro.phone.set()
@dp.callback_query_handler(text='ortga',state=Yuk_buxoro.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Yuk_buxoro.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_buxoro.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Yuk_buxoro.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Yuk_buxoro.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Yuk_buxoro.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Yuk_buxoro.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_buxoro.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Yuk_buxoro.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_buxoro.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Yuk_buxoro.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Yuk_buxoro.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_buxoro.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_buxoro.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Yuk_buxoro.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Yuk_buxoro.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
        ...
        if i[2] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='8d5sdsdff9w9r52e6w448r88r'))
...
@dp.callback_query_handler(text="8d5sdsdff9w9r52e6w448r88r")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Yuk_buxoro.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Qanday yuk mashinasi kerak ? ",reply_markup= qanaqa_avto)
    await Yuk_buxoro.xa_yoq.set()
@dp.callback_query_handler(state=Yuk_buxoro.xa_yoq)
async def kisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_buxoro.turi.set()
@dp.message_handler(state=Yuk_buxoro.turi)
async def yuk_turi(message:Message,state:FSMContext):
    ...
    await Yuk_buxoro.narx.set()
@dp.message_handler(state=Yuk_buxoro.narx)
async def yuk_narx(message:Message,state:FSMContext):
    ...
    await Yuk_buxoro.end.set()
@dp.callback_query_handler(text='Confirm',state=Yuk_buxoro.end)
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='dfghsqwqwjklyhgfd'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
...
@dp.callback_query_handler(text="dfghsqwqwjklyhgfd")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)

@dp.callback_query_handler(text='UnConfirm', state=Yuk_buxoro.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()