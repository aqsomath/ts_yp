...
from states.yuk_states import Yuk_sirdaryo
...
@dp.callback_query_handler(yuk_callback.filter(item_name='6666'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan yuborasiz ? ",reply_markup=sirdaryo_yol)
    await Yuk_sirdaryo.tuman.set()
# 1 - ORTGA
@dp.callback_query_handler(text='ortga',state=Yuk_sirdaryo.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
...
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Yuk_sirdaryo.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Yuk_sirdaryo.tuman)
async def tumanx(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_sirdaryo.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Yuk_sirdaryo.viloyatga)
async def ztumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan yuborasiz ? ", reply_markup=sirdaryo_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Yuk_sirdaryo.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_sirdaryo.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_sirdaryo.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_sirdaryo.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Yuk_sirdaryo.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_sirdaryo.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_sirdaryo.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_sirdaryo.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Yuk_sirdaryo.soat.set()
@dp.callback_query_handler(text='ortga',state=Yuk_sirdaryo.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_sirdaryo.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_sirdaryo.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_sirdaryo.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_sirdaryo.phone.set()
@dp.callback_query_handler(text='ortga',state=Yuk_sirdaryo.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Yuk_sirdaryo.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_sirdaryo.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Yuk_sirdaryo.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Yuk_sirdaryo.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Yuk_sirdaryo.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Yuk_sirdaryo.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_sirdaryo.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Yuk_sirdaryo.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_sirdaryo.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Yuk_sirdaryo.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Yuk_sirdaryo.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_sirdaryo.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_sirdaryo.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Yuk_sirdaryo.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Yuk_sirdaryo.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
        ...
        if i[2] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='A4AS65D4AS65D'))
...
@dp.callback_query_handler(text="A4AS65D4AS65D")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Yuk_sirdaryo.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Qanday yuk mashinasi kerak ? ",reply_markup= qanaqa_avto)
    await Yuk_sirdaryo.xa_yoq.set()
@dp.callback_query_handler(state=Yuk_sirdaryo.xa_yoq)
async def kisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_sirdaryo.turi.set()
@dp.message_handler(state=Yuk_sirdaryo.turi)
async def yuk_turi(message:Message,state:FSMContext):
    ...
    await Yuk_sirdaryo.narx.set()
@dp.message_handler(state=Yuk_sirdaryo.narx)
async def yuk_narx(message:Message,state:FSMContext):
    ...
    await Yuk_sirdaryo.end.set()
@dp.callback_query_handler(text='Confirm',state=Yuk_sirdaryo.end)
...
@dp.callback_query_handler(text="97SDSE5E5E55E")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Yuk_sirdaryo.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()