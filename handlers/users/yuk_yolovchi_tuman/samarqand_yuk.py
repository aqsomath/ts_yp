...
from states.yuk_states import Yuk_samarqand
...
@dp.callback_query_handler(yuk_callback.filter(item_name='131313'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan yuborasiz ? ",reply_markup=samarqand_yol)
    await Yuk_samarqand.tuman.set()
# 1 - ORTGA
@dp.callback_query_handler(text='ortga',state=Yuk_samarqand.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Yukni qaysi viloyatdan yuborasiz ?", reply_markup=yuk_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Yuk_samarqand.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Yuk_samarqand.tuman)
async def tumanx(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_samarqand.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Yuk_samarqand.viloyatga)
async def ztumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan yuborasiz ? ", reply_markup=samarqand_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Yuk_samarqand.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_samarqand.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_samarqand.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_samarqand.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Yuk_samarqand.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_samarqand.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_samarqand.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Yuk_samarqand.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Yuk_samarqand.soat.set()
@dp.callback_query_handler(text='ortga',state=Yuk_samarqand.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga yuk yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Yuk_samarqand.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_samarqand.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Yuk_samarqand.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_samarqand.phone.set()
@dp.callback_query_handler(text='ortga',state=Yuk_samarqand.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Yuk_samarqand.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_samarqand.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Yuk_samarqand.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Yuk_samarqand.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Yuk_samarqand.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Yuk_samarqand.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_samarqand.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Yuk_samarqand.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_samarqand.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Yuk_samarqand.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Yuk_samarqand.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_samarqand.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Yuk_samarqand.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Yuk_samarqand.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Yuk_samarqand.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
        ...
        if i[2] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='5D5555D55D5W5'))
...
@dp.callback_query_handler(text="5D5555D55D5W5")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Yuk_samarqand.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Qanday yuk mashinasi kerak ? ",reply_markup= qanaqa_avto)
    await Yuk_samarqand.xa_yoq.set()
@dp.callback_query_handler(state=Yuk_samarqand.xa_yoq)
async def kisi(call:CallbackQuery,state:FSMContext):
    ...
    await Yuk_samarqand.turi.set()
@dp.message_handler(state=Yuk_samarqand.turi)
async def yuk_turi(message:Message,state:FSMContext):
    ...
    await Yuk_samarqand.narx.set()
@dp.message_handler(state=Yuk_samarqand.narx)
async def yuk_narx(message:Message,state:FSMContext):
    ...
    await Yuk_samarqand.end.set()
@dp.callback_query_handler(text='Confirm',state=Yuk_samarqand.end)
...
@dp.callback_query_handler(text="QQ8W9Q8W9Q")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)

@dp.callback_query_handler(text='UnConfirm', state=Yuk_samarqand.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()