...
async def andijontuman(call:CallbackQuery):
    await db.add_driver(region="Andijon_hammasi", telegram_id=call.from_user.id)
...
async def edit(call:CallbackQuery):
    await db.update_driver("Andijon_hammasi_1", call.from_user.id)
...
async def edit(call:CallbackQuery):
    await db.add_driver("ulug'nor", call.from_user.id)
    andijon_old['inline_keyboard'][0][0]['text'] = "✅ Ulug'nor"
