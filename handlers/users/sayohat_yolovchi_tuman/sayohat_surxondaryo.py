...
from states.sayohat_states import Sayohat_surx
from utils.misc import show_on_gmaps


@dp.callback_query_handler(sayohat_callback.filter(item_name='yakshanba'))
async def andijon_sayohat(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Surxondaryoning qaysi tumanidan yo'lga sayohatga yo'l olasiz ?", reply_markup=surxondaryo_yol)
    await Sayohat_surx.tuman.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_surx.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_surx.tuman)
...
@dp.callback_query_handler(state=Sayohat_surx.tuman)
async def sayohat_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_surx.sayohat_manzillari.set()


@dp.message_handler(state=Sayohat_surx.sayohat_manzillari)
async def manzillar(message:Message,state:FSMContext):
    ...
    await Sayohat_surx.sana.set()

@dp.message_handler(state=Sayohat_surx.sana)
async def manzillar(message:Message,state:FSMContext):
    await state.update_data(
        {
            "sana":message.text
    ...
    await Sayohat_surx.soat.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_surx.soat)
...
async def sayohat_soati(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_surx.phone.set()
@dp.message_handler(content_types=['contact','text'],state=Sayohat_surx.phone)
async def kont(message:Message,state:FSMContext):
    ...
    await Sayohat_surx.tasdiqlash.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_surx.tasdiqlash)
async def ett(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga kerakli xizmat turini tanlang", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_surx.tasdiqlash)
...
@dp.message_handler(content_types='location',state=Sayohat_surx.tasdiqlash)
async def lokatsiya_sayohat(message:Message,state:FSMContext):
    ...
    await Sayohat_surx.xa_yoq.set()

@dp.callback_query_handler(text='yingisi',state=Sayohat_surx.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Sayohat_surx.xa_yoq)
        ...
        if i[5] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='sasdsdewewerfdsd'))
...
async def kuns(message:Message,state:FSMContext):
    ...
    await Sayohat_surx.necha_kishi.set()

@dp.message_handler(state=Sayohat_surx.necha_kishi)
async def necha_sayyoh(message:Message,state:FSMContext):
    ...
    await Sayohat_surx.qancha_narx.set()
@dp.message_handler(state=Sayohat_surx.qancha_narx)
async def narx_qancha(message:Message,state:FSMContext):
    ...
    await Sayohat_surx.end.set()
@dp.callback_query_handler(text='Confirm',state=Sayohat_surx.end)
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='sawefdvccsvxcdvvbcbbbc'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
...
@dp.callback_query_handler(text="sawefdvccsvxcdvvbcbbbc")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Sayohat_surx.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
