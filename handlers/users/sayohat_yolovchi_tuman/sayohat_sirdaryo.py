...
from states.sayohat_states import Sayohat_sir
from utils.misc import show_on_gmaps


@dp.callback_query_handler(sayohat_callback.filter(item_name='shanba'))
async def andijon_sayohat(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Sirdaryoning qaysi tumanidan yo'lga sayohatga yo'l olasiz ?", reply_markup=sirdaryo_yol)
    await Sayohat_sir.tuman.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_sir.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_sir.tuman)
...
@dp.callback_query_handler(state=Sayohat_sir.tuman)
async def sayohat_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_sir.sayohat_manzillari.set()


@dp.message_handler(state=Sayohat_sir.sayohat_manzillari)
async def manzillar(message:Message,state:FSMContext):
    ...
    await Sayohat_sir.sana.set()

@dp.message_handler(state=Sayohat_sir.sana)
async def manzillar(message:Message,state:FSMContext):
    await state.update_data(
        {
            "sana":message.text
    ...
    await Sayohat_sir.soat.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_sir.soat)
...
async def sayohat_soati(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_sir.phone.set()
@dp.message_handler(content_types=['contact','text'],state=Sayohat_sir.phone)
async def kont(message:Message,state:FSMContext):
    ...
    await Sayohat_sir.tasdiqlash.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_sir.tasdiqlash)
async def ett(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga kerakli xizmat turini tanlang", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_sir.tasdiqlash)
...
@dp.message_handler(content_types='location',state=Sayohat_sir.tasdiqlash)
async def lokatsiya_sayohat(message:Message,state:FSMContext):
    ...
    await Sayohat_sir.xa_yoq.set()

@dp.callback_query_handler(text='yingisi',state=Sayohat_sir.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Sayohat_sir.xa_yoq)
        ...
        if i[5] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='zppsoappai'))
...
async def kuns(message:Message,state:FSMContext):
    ...
    await Sayohat_sir.necha_kishi.set()

@dp.message_handler(state=Sayohat_sir.necha_kishi)
async def necha_sayyoh(message:Message,state:FSMContext):
    ...
    await Sayohat_sir.qancha_narx.set()
@dp.message_handler(state=Sayohat_sir.qancha_narx)
async def narx_qancha(message:Message,state:FSMContext):
    ...
    await Sayohat_sir.end.set()
@dp.callback_query_handler(text='Confirm',state=Sayohat_sir.end)
...
@dp.callback_query_handler(text="qpowoduuebn")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Sayohat_sir.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
