...
from states.sayohat_states import Sayohat_sam
from utils.misc import show_on_gmaps


@dp.callback_query_handler(sayohat_callback.filter(item_name='oktabr'))
async def andijon_sayohat(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Samarqandning qaysi tumanidan yo'lga sayohatga yo'l olasiz ?", reply_markup=samarqand_yol)
    await Sayohat_sam.tuman.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_sam.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_sam.tuman)
...
@dp.callback_query_handler(state=Sayohat_sam.tuman)
async def sayohat_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_sam.sayohat_manzillari.set()


@dp.message_handler(state=Sayohat_sam.sayohat_manzillari)
async def manzillar(message:Message,state:FSMContext):
    ...
    await Sayohat_sam.sana.set()

@dp.message_handler(state=Sayohat_sam.sana)
async def manzillar(message:Message,state:FSMContext):
    await state.update_data(
        {
            "sana":message.text
    ...
    await Sayohat_sam.soat.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_sam.soat)
...
async def sayohat_soati(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_sam.phone.set()
@dp.message_handler(content_types=['contact','text'],state=Sayohat_sam.phone)
async def kont(message:Message,state:FSMContext):
    ...
    await Sayohat_sam.tasdiqlash.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_sam.tasdiqlash)
async def ett(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga kerakli xizmat turini tanlang", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_sam.tasdiqlash)
...
@dp.message_handler(content_types='location',state=Sayohat_sam.tasdiqlash)
async def lokatsiya_sayohat(message:Message,state:FSMContext):
    ...
    await Sayohat_sam.xa_yoq.set()

@dp.callback_query_handler(text='yingisi',state=Sayohat_sam.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Sayohat_sam.xa_yoq)
        ...
        if i[5] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='8787dssfkjkjkjeiuri'))
...
async def kuns(message:Message,state:FSMContext):
    ...
    await Sayohat_sam.necha_kishi.set()

@dp.message_handler(state=Sayohat_sam.necha_kishi)
async def necha_sayyoh(message:Message,state:FSMContext):
    ...
    await Sayohat_sam.qancha_narx.set()
@dp.message_handler(state=Sayohat_sam.qancha_narx)
async def narx_qancha(message:Message,state:FSMContext):
    ...
    await Sayohat_sam.end.set()
@dp.callback_query_handler(text='Confirm',state=Sayohat_sam.end)
...
@dp.callback_query_handler(text="wqoopepi")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Sayohat_sam.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
