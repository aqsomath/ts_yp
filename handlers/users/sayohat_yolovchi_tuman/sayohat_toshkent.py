...
from states.sayohat_states import Sayohat_tosh
from utils.misc import show_on_gmaps


@dp.callback_query_handler(sayohat_callback.filter(item_name='juma'))
async def andijon_sayohat(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Toshkentning qaysi tumanidan yo'lga sayohatga yo'l olasiz ?", reply_markup=toshkent_yol)
    await Sayohat_tosh.tuman.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_tosh.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_tosh.tuman)
...
@dp.callback_query_handler(state=Sayohat_tosh.tuman)
async def sayohat_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_tosh.sayohat_manzillari.set()


@dp.message_handler(state=Sayohat_tosh.sayohat_manzillari)
async def manzillar(message:Message,state:FSMContext):
    ...
    await Sayohat_tosh.sana.set()

@dp.message_handler(state=Sayohat_tosh.sana)
async def manzillar(message:Message,state:FSMContext):
    await state.update_data(
        {
            "sana":message.text
    ...
    await Sayohat_tosh.soat.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_tosh.soat)
...
async def sayohat_soati(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_tosh.phone.set()
@dp.message_handler(content_types=['contact','text'],state=Sayohat_tosh.phone)
async def kont(message:Message,state:FSMContext):
    ...
    await Sayohat_tosh.tasdiqlash.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_tosh.tasdiqlash)
async def ett(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga kerakli xizmat turini tanlang", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_tosh.tasdiqlash)
...
@dp.message_handler(content_types='location',state=Sayohat_tosh.tasdiqlash)
async def lokatsiya_sayohat(message:Message,state:FSMContext):
    ...
    await Sayohat_tosh.xa_yoq.set()

@dp.callback_query_handler(text='yingisi',state=Sayohat_tosh.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Sayohat_tosh.xa_yoq)
        ...
        if i[5] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='65565fvfdfsdfdf'))
...
async def kuns(message:Message,state:FSMContext):
    ...
    await Sayohat_tosh.necha_kishi.set()

@dp.message_handler(state=Sayohat_tosh.necha_kishi)
async def necha_sayyoh(message:Message,state:FSMContext):
    ...
    await Sayohat_tosh.qancha_narx.set()
@dp.message_handler(state=Sayohat_tosh.qancha_narx)
async def narx_qancha(message:Message,state:FSMContext):
    ...
    await Sayohat_tosh.end.set()
@dp.callback_query_handler(text='Confirm',state=Sayohat_tosh.end)
...
@dp.callback_query_handler(text="uvjid")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)

@dp.callback_query_handler(text='UnConfirm', state=Sayohat_tosh.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
