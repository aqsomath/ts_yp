...
from states.sayohat_states import Sayohat_namangan
from utils.misc import show_on_gmaps


@dp.callback_query_handler(sayohat_callback.filter(item_name='seshanba'))
async def andijon_sayohat(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Namanganning qaysi tumanidan yo'lga sayohatga yo'l olasiz ?", reply_markup=namangan_yol)
    await Sayohat_namangan.tuman.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_namangan.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_namangan.tuman)
...
@dp.callback_query_handler(state=Sayohat_namangan.tuman)
async def sayohat_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_namangan.sayohat_manzillari.set()


@dp.message_handler(state=Sayohat_namangan.sayohat_manzillari)
async def manzillar(message:Message,state:FSMContext):
    ...
    await Sayohat_namangan.sana.set()

@dp.message_handler(state=Sayohat_namangan.sana)
async def manzillar(message:Message,state:FSMContext):
    await state.update_data(
        {
            "sana":message.text
    ...
    await Sayohat_namangan.soat.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_namangan.soat)
async def times(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_namangan.soat)
async def comeback(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sayohat qiladigan sanangizni kiriting :")
    await Sayohat_namangan.sana.set()

@dp.callback_query_handler(state=Sayohat_namangan.soat)
async def sayohat_soati(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_namangan.phone.set()
@dp.message_handler(content_types=['contact','text'],state=Sayohat_namangan.phone)
async def kont(message:Message,state:FSMContext):
    ...
    await Sayohat_namangan.tasdiqlash.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_namangan.tasdiqlash)
async def ett(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga kerakli xizmat turini tanlang", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_namangan.tasdiqlash)
...
@dp.message_handler(content_types='location',state=Sayohat_namangan.tasdiqlash)
async def lokatsiya_sayohat(message:Message,state:FSMContext):
    ...
    await Sayohat_namangan.xa_yoq.set()

@dp.callback_query_handler(text='yingisi',state=Sayohat_namangan.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Sayohat_namangan.xa_yoq)
        ...
        if i[5] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='dd4v654vv64b'))
...
@dp.callback_query_handler(text="dd4v654vv64b")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='nott', state=Sayohat_namangan.xa_yoq)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text='add_information', state=Sayohat_namangan.xa_yoq)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Necha kunlik sayohat qilmoqchisiz ? ")
    await Sayohat_namangan.necha_kun.set()
@dp.message_handler(state=Sayohat_namangan.necha_kun)
async def kuns(message:Message,state:FSMContext):
    ...
    await Sayohat_namangan.necha_kishi.set()

@dp.message_handler(state=Sayohat_namangan.necha_kishi)
async def necha_sayyoh(message:Message,state:FSMContext):
    ...
    await Sayohat_namangan.qancha_narx.set()
@dp.message_handler(state=Sayohat_namangan.qancha_narx)
async def narx_qancha(message:Message,state:FSMContext):
    ...
    await Sayohat_namangan.end.set()
@dp.callback_query_handler(text='Confirm',state=Sayohat_namangan.end)
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='qw5487d8s7d85ds45d'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
...
@dp.callback_query_handler(text="qw5487d8s7d85ds45d")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Sayohat_namangan.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
