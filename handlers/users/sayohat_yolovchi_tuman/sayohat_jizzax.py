...
from states.sayohat_states import Sayohat_jizzax
from utils.misc import show_on_gmaps


@dp.callback_query_handler(sayohat_callback.filter(item_name='sentabr'))
async def andijon_sayohat(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Jizzaxning qaysi tumanidan yo'lga sayohatga yo'l olasiz ?", reply_markup=jizzax_yol)
    await Sayohat_jizzax.tuman.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_jizzax.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_jizzax.tuman)
...
@dp.callback_query_handler(state=Sayohat_jizzax.tuman)
async def sayohat_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_jizzax.sayohat_manzillari.set()


@dp.message_handler(state=Sayohat_jizzax.sayohat_manzillari)
async def manzillar(message:Message,state:FSMContext):
    ...
    await Sayohat_jizzax.sana.set()

@dp.message_handler(state=Sayohat_jizzax.sana)
async def manzillar(message:Message,state:FSMContext):
    await state.update_data(
        {
            "sana":message.text
    ...
    await Sayohat_jizzax.soat.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_jizzax.soat)
...
async def sayohat_soati(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_jizzax.phone.set()
@dp.message_handler(content_types=['contact','text'],state=Sayohat_jizzax.phone)
async def kont(message:Message,state:FSMContext):
    ...
    await Sayohat_jizzax.tasdiqlash.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_jizzax.tasdiqlash)
async def ett(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga kerakli xizmat turini tanlang", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_jizzax.tasdiqlash)
...
@dp.message_handler(content_types='location',state=Sayohat_jizzax.tasdiqlash)
async def lokatsiya_sayohat(message:Message,state:FSMContext):
    ...
    await Sayohat_jizzax.xa_yoq.set()

@dp.callback_query_handler(text='yingisi',state=Sayohat_jizzax.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Sayohat_jizzax.xa_yoq)
        ...
        if i[5] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='98d956vbgbnh5n45h4nj65n6g6g56'))
...
@dp.callback_query_handler(text="98d956vbgbnh5n45h4nj65n6g6g56")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='nott', state=Sayohat_jizzax.xa_yoq)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text='add_information', state=Sayohat_jizzax.xa_yoq)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Necha kunlik sayohat qilmoqchisiz ? ")
    await Sayohat_jizzax.necha_kun.set()
@dp.message_handler(state=Sayohat_jizzax.necha_kun)
async def kuns(message:Message,state:FSMContext):
    ...
    await Sayohat_jizzax.necha_kishi.set()

@dp.message_handler(state=Sayohat_jizzax.necha_kishi)
async def necha_sayyoh(message:Message,state:FSMContext):
    ...
    await Sayohat_jizzax.qancha_narx.set()
@dp.message_handler(state=Sayohat_jizzax.qancha_narx)
async def narx_qancha(message:Message,state:FSMContext):
    ...
    await Sayohat_jizzax.end.set()
@dp.callback_query_handler(text='Confirm',state=Sayohat_jizzax.end)
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='sdhjvgfhjgvygvyrmmn'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
...
@dp.callback_query_handler(text="sdhjvgfhjgvygvyrmmn")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Sayohat_jizzax.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
