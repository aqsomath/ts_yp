...
from states.sayohat_states import Sayohat_fargona
from utils.misc import show_on_gmaps


@dp.callback_query_handler(sayohat_callback.filter(item_name='chorshanba'))
async def andijon_sayohat(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Farg'onaning qaysi tumanidan yo'lga sayohatga yo'l olasiz ?", reply_markup=fargona_yol)
    await Sayohat_fargona.tuman.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_fargona.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_fargona.tuman)
...
@dp.callback_query_handler(state=Sayohat_fargona.tuman)
async def sayohat_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_fargona.sayohat_manzillari.set()


@dp.message_handler(state=Sayohat_fargona.sayohat_manzillari)
async def manzillar(message:Message,state:FSMContext):
    ...
    await Sayohat_fargona.sana.set()

@dp.message_handler(state=Sayohat_fargona.sana)
async def manzillar(message:Message,state:FSMContext):
    await state.update_data(
        {
            "sana":message.text
    ...
    await Sayohat_fargona.soat.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_fargona.soat)
...
async def sayohat_soati(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_fargona.phone.set()
@dp.message_handler(content_types=['contact','text'],state=Sayohat_fargona.phone)
async def kont(message:Message,state:FSMContext):
    ...
    await Sayohat_fargona.tasdiqlash.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_fargona.tasdiqlash)
async def ett(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga kerakli xizmat turini tanlang", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_fargona.tasdiqlash)
...
@dp.message_handler(content_types='location',state=Sayohat_fargona.tasdiqlash)
async def lokatsiya_sayohat(message:Message,state:FSMContext):
    ...
    await Sayohat_fargona.xa_yoq.set()

@dp.callback_query_handler(text='yingisi',state=Sayohat_fargona.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Sayohat_fargona.xa_yoq)
        ...
        if i[5] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='rtyushncmmncsudygfhjd'))
...
@dp.callback_query_handler(text="rtyushncmmncsudygfhjd")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='nott', state=Sayohat_fargona.xa_yoq)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text='add_information', state=Sayohat_fargona.xa_yoq)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Necha kunlik sayohat qilmoqchisiz ? ")
    await Sayohat_fargona.necha_kun.set()
@dp.message_handler(state=Sayohat_fargona.necha_kun)
async def kuns(message:Message,state:FSMContext):
    ...
    await Sayohat_fargona.necha_kishi.set()

@dp.message_handler(state=Sayohat_fargona.necha_kishi)
async def necha_sayyoh(message:Message,state:FSMContext):
    ...
    await Sayohat_fargona.qancha_narx.set()
@dp.message_handler(state=Sayohat_fargona.qancha_narx)
async def narx_qancha(message:Message,state:FSMContext):
    ...
    await Sayohat_fargona.end.set()
@dp.callback_query_handler(text='Confirm',state=Sayohat_fargona.end)
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='sgfhgfsghjamjncmcmbmxcb'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
...
@dp.callback_query_handler(text="sgfhgfsghjamjncmcmbmxcb")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Sayohat_fargona.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
