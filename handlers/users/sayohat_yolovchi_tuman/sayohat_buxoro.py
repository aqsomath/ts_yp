...
from states.sayohat_states import Sayohat_buxoro
from utils.misc import show_on_gmaps


@dp.callback_query_handler(sayohat_callback.filter(item_name='payshanba'))
async def andijon_sayohat(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Buxoroning qaysi tumanidan yo'lga sayohatga yo'l olasiz ?", reply_markup=buxoro_yol)
    await Sayohat_buxoro.tuman.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_buxoro.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_buxoro.tuman)
...
@dp.callback_query_handler(state=Sayohat_buxoro.tuman)
async def sayohat_tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_buxoro.sayohat_manzillari.set()


@dp.message_handler(state=Sayohat_buxoro.sayohat_manzillari)
async def manzillar(message:Message,state:FSMContext):
    ...
    await Sayohat_buxoro.sana.set()

@dp.message_handler(state=Sayohat_buxoro.sana)
async def manzillar(message:Message,state:FSMContext):
    await state.update_data(
        {
            "sana":message.text
    ...
    await Sayohat_buxoro.soat.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_buxoro.soat)
...
async def sayohat_soati(call:CallbackQuery,state:FSMContext):
    ...
    await Sayohat_buxoro.phone.set()
@dp.message_handler(content_types=['contact','text'],state=Sayohat_buxoro.phone)
async def kont(message:Message,state:FSMContext):
    ...
    await Sayohat_buxoro.tasdiqlash.set()
@dp.callback_query_handler(text='atmen',state=Sayohat_buxoro.tasdiqlash)
async def ett(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga kerakli xizmat turini tanlang", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='ortga',state=Sayohat_buxoro.tasdiqlash)
...
@dp.message_handler(content_types='location',state=Sayohat_buxoro.tasdiqlash)
async def lokatsiya_sayohat(message:Message,state:FSMContext):
    ...
    await Sayohat_buxoro.xa_yoq.set()

@dp.callback_query_handler(text='yingisi',state=Sayohat_buxoro.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Sayohat_buxoro.xa_yoq)
        ...
        if i[5] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='8d5ff9w9sd68789d6x3s46cr52e6w448r88r'))
...
@dp.callback_query_handler(text="8d5ff9w9sd68789d6x3s46cr52e6w448r88r")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='nott', state=Sayohat_buxoro.xa_yoq)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text='add_information', state=Sayohat_buxoro.xa_yoq)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Necha kunlik sayohat qilmoqchisiz ? ")
    await Sayohat_buxoro.necha_kun.set()
@dp.message_handler(state=Sayohat_buxoro.necha_kun)
async def kuns(message:Message,state:FSMContext):
    ...
    await Sayohat_buxoro.necha_kishi.set()

@dp.message_handler(state=Sayohat_buxoro.necha_kishi)
async def necha_sayyoh(message:Message,state:FSMContext):
    ...
    await Sayohat_buxoro.qancha_narx.set()
@dp.message_handler(state=Sayohat_buxoro.qancha_narx)
async def narx_qancha(message:Message,state:FSMContext):
    ...
    await Sayohat_buxoro.end.set()
@dp.callback_query_handler(text='Confirm',state=Sayohat_buxoro.end)
...
@dp.callback_query_handler(text="as4s55wss545as")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Sayohat_buxoro.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
