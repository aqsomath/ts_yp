...
from states.yolovchi_reys_states import Yolovchi_fargon
...
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text_contains="farg'ona",state=None)
async def adnijonyolovchi(call:CallbackQuery,state: FSMContext):
    await state.update_data(
        {"viloyat":"Farg'ona"}
    )
    await call.message.answer("Qaysi tumandan ketasiz :",reply_markup=fargona_yol)
    await Yolovchi_fargon.viloyatga.set()
# 2 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_fargon.viloyatga)
async def orqaga(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
# 2 -  BEKOR QILISH
@dp.callback_query_handler(text='atmen',state=Yolovchi_fargon.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(state=Yolovchi_fargon.viloyatga)
async def tuman_andijon(call:CallbackQuery, state:FSMContext):
    ...
    await Yolovchi_fargon.tumaniga.set()
@dp.callback_query_handler(text='atmen',state=Yolovchi_fargon.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
#  3 - ORTGA
@dp.callback_query_handler(text='ortga',state=Yolovchi_fargon.tumaniga)
async def nazad(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumandan ketasiz :", reply_markup=fargona_yol)
    await Yolovchi_fargon.viloyatga.set()

@dp.callback_query_handler(state=Yolovchi_fargon.tumaniga)
async def tuman_andijon(call:CallbackQuery, state:FSMContext):
    data = call.data
    ...
    await Yolovchi_fargon.soat.set()


#  4 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_fargon.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
# 4 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_fargon.soat)
async def ketish_soati(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz? ", reply_markup=viloyatlar_yol_x)
    await Yolovchi_fargon.tumaniga.set()


@dp.callback_query_handler(state=Yolovchi_fargon.soat)
async def ketish_soati(call: CallbackQuery, state: FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchida yo'lga chiqmoqchisiz ?", reply_markup=time)
    await Yolovchi_fargon.phone.set()


#   5 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_fargon.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
# 5 - ORTGA
@dp.callback_query_handler(time_callback.filter(item_name='ortga'), state=Yolovchi_fargon.phone)
async def phone(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz? ", reply_markup=viloyatlar_yol_x)
    await Yolovchi_fargon.tumaniga.set()


@dp.callback_query_handler(state=Yolovchi_fargon.phone)
async def phone(call: CallbackQuery, state: FSMContext):
    print(call.data)
    await state.update_data(
        {
    ...
    await Yolovchi_fargon.locatsiya.set()


#  6 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_fargon.locatsiya)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
#  6 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_fargon.locatsiya)
async def phone(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqmoqchisiz ?", reply_markup=time)
    await Yolovchi_fargon.phone.set()


@dp.message_handler(content_types=['contact', 'text'], state=Yolovchi_fargon.locatsiya)
async def location(message: Message, state: FSMContext):
    ...
    await Yolovchi_fargon.tasdiqlash.set()


#     7 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_fargon.tasdiqlash)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
#     7 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_fargon.tasdiqlash)
...
@dp.message_handler(content_types=['location'], state=Yolovchi_fargon.tasdiqlash)
async def location(message: Message, state: FSMContext):
    ...
    await Yolovchi_fargon.xa_yoq.set()


#     8 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_fargon.xa_yoq)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
#     8 - ORTGA
@dp.callback_query_handler(text="ortga", state=Yolovchi_fargon.xa_yoq)
...
@dp.callback_query_handler(text="yingisi", state=Yolovchi_fargon.tasdiqlash)
async def keyin(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(text='yesss', state=Yolovchi_fargon.xa_yoq)
        ...
        if i[1] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='aqwqws54545'))
...
async def kisi(call: CallbackQuery, state: FSMContext):
    ...
    await Yolovchi_fargon.oldi_kerakmi.set()


@dp.callback_query_handler(state=Yolovchi_fargon.oldi_kerakmi)
async def kisi(call: CallbackQuery, state: FSMContext):
    await state.update_data(
        {
            "oldi_joy": call.data
    ...
    await Yolovchi_fargon.qanday_moshina.set()


@dp.callback_query_handler(state=Yolovchi_fargon.qanday_moshina)
async def kisi(call: CallbackQuery, state: FSMContext):
    await state.update_data(
        {
            "Avto_turi": call.data
    ...
    await Yolovchi_fargon.end.set()


@dp.callback_query_handler(text='Confirm', state=Yolovchi_fargon.end)
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='as4x4465s465as465'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
                              "Ularning bog'lanishini kuting !\n", reply_markup=umumiy_menu
                              )
    await state.reset_state(with_data=False)
@dp.callback_query_handler(text="as4x4465s465as465")
async def qabul_qilish(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)

@dp.callback_query_handler(text='UnConfirm', state=Yolovchi_fargon.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
