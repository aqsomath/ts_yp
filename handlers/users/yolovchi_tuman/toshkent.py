...
from states.yolovchi_reys_states import Yolovchi_toshkent
...
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text_contains="toshkent",state=None)
async def adnijonyolovchi(call:CallbackQuery,state: FSMContext):
    await state.update_data(
        {"viloyat":"Toshkent"}
    )
    await call.message.answer("Qaysi tumandan ketasiz :",reply_markup=toshkent_yol)
    await Yolovchi_toshkent.viloyatga.set()
# 2 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_toshkent.viloyatga)
async def orqaga(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
# 2 -  BEKOR QILISH
@dp.callback_query_handler(text='atmen',state=Yolovchi_toshkent.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(state=Yolovchi_toshkent.viloyatga)
async def tuman_andijon(call:CallbackQuery, state:FSMContext):
    ...
    await Yolovchi_toshkent.tumaniga.set()
@dp.callback_query_handler(text='atmen',state=Yolovchi_toshkent.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
#  3 - ORTGA
@dp.callback_query_handler(text='ortga',state=Yolovchi_toshkent.tumaniga)
async def nazad(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumandan ketasiz :", reply_markup=toshkent_yol)
    await Yolovchi_toshkent.viloyatga.set()

@dp.callback_query_handler(state=Yolovchi_toshkent.tumaniga)
async def tuman_andijon(call:CallbackQuery, state:FSMContext):
    data = call.data
    ...
    await Yolovchi_toshkent.soat.set()


#  4 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_toshkent.soat)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
# 4 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_toshkent.soat)
async def ketish_soati(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz? ", reply_markup=viloyatlar_yol_x)
    await Yolovchi_toshkent.tumaniga.set()


@dp.callback_query_handler(state=Yolovchi_toshkent.soat)
async def ketish_soati(call: CallbackQuery, state: FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchida yo'lga chiqmoqchisiz ?", reply_markup=time)
    await Yolovchi_toshkent.phone.set()


#   5 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_toshkent.phone)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
# 5 - ORTGA
@dp.callback_query_handler(time_callback.filter(item_name='ortga'), state=Yolovchi_toshkent.phone)
async def phone(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz? ", reply_markup=viloyatlar_yol_x)
    await Yolovchi_toshkent.tumaniga.set()


@dp.callback_query_handler(state=Yolovchi_toshkent.phone)
async def phone(call: CallbackQuery, state: FSMContext):
    print(call.data)
    await state.update_data(
        {
    ...
    await Yolovchi_toshkent.locatsiya.set()


#  6 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_toshkent.locatsiya)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
#  6 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_toshkent.locatsiya)
async def phone(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqmoqchisiz ?", reply_markup=time)
    await Yolovchi_toshkent.phone.set()


@dp.message_handler(content_types=['contact', 'text'], state=Yolovchi_toshkent.locatsiya)
async def location(message: Message, state: FSMContext):
    ...
    await Yolovchi_toshkent.tasdiqlash.set()


#     7 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_toshkent.tasdiqlash)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
#     7 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_toshkent.tasdiqlash)
...
@dp.message_handler(content_types=['location'], state=Yolovchi_toshkent.tasdiqlash)
async def location(message: Message, state: FSMContext):
    ...
    await Yolovchi_toshkent.xa_yoq.set()


#     8 - BEKOR QILISH
@dp.callback_query_handler(text='atmen', state=Yolovchi_toshkent.xa_yoq)
async def haydovchi(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
#     8 - ORTGA
@dp.callback_query_handler(text="ortga", state=Yolovchi_toshkent.xa_yoq)
...
@dp.callback_query_handler(text="yingisi", state=Yolovchi_toshkent.tasdiqlash)
async def keyin(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(text='yesss', state=Yolovchi_toshkent.xa_yoq)
        ...
        if i[1] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='iuikjncd'))
...
@dp.callback_query_handler(text="iuikjncd")
...
@dp.callback_query_handler(text='nott', state=Yolovchi_toshkent.xa_yoq)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(text='add_information', state=Yolovchi_toshkent.xa_yoq)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Necha kishisizlar ? ", reply_markup=necha_kishisizlar)
    await Yolovchi_toshkent.necha_kishi.set()


@dp.callback_query_handler(state=Yolovchi_toshkent.necha_kishi)
async def kisi(call: CallbackQuery, state: FSMContext):
    ...
    await Yolovchi_toshkent.oldi_kerakmi.set()


@dp.callback_query_handler(state=Yolovchi_toshkent.oldi_kerakmi)
async def kisi(call: CallbackQuery, state: FSMContext):
    await state.update_data(
        {
            "oldi_joy": call.data
    ...
    await Yolovchi_toshkent.qanday_moshina.set()


@dp.callback_query_handler(state=Yolovchi_toshkent.qanday_moshina)
async def kisi(call: CallbackQuery, state: FSMContext):
    await state.update_data(
        {
            "Avto_turi": call.data
    ...
    await Yolovchi_toshkent.end.set()


@dp.callback_query_handler(text='Confirm', state=Yolovchi_toshkent.end)
...
@dp.callback_query_handler(text="55sdd6df6df")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Yolovchi_toshkent.end)
async def y_n(call: CallbackQuery, state: FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()
