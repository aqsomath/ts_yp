...
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(text="andijon",state=None)
async def adnijonyolovchi(call:CallbackQuery,state: FSMContext):
    await state.update_data(
        {"viloyat":"Andijon"}
    )
    await call.message.answer("Qaysi tumandan ketasiz :",reply_markup=andijon_yol)
    await Yolovchi_andijon.viloyatga.set()
# 2 - ORTGA
@dp.callback_query_handler(text='ortga', state=Yolovchi_andijon.viloyatga)
async def orqaga(call:CallbackQuery,state:FSMContext):
    ...
    await state.finish()
# 2 -  BEKOR QILISH
@dp.callback_query_handler(text='atmen',state=Yolovchi_andijon.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
...
@dp.callback_query_handler(text='atmen',state=Yolovchi_andijon.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
#  3 - ORTGA
...
#  4 - BEKOR QILISH
@dp.callback_query_handler(text='atmen',state=Yolovchi_andijon.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
# 4 - ORTGA
...
#   5 - BEKOR QILISH
@dp.callback_query_handler(text='atmen',state=Yolovchi_andijon.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
# 5 - ORTGA
@dp.callback_query_handler(time_callback.filter(item_name='ortga'),state=Yolovchi_andijon.phone)
async def phone(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Qaysi viloyatga borasiz? ", reply_markup=viloyatlar_yol_x)
    await Yolovchi_andijon.tumaniga.set()
@dp.callback_query_handler(state=Yolovchi_andijon.phone)
async def phone(call:CallbackQuery, state:FSMContext):
    print(call.data)
    await state.update_data(
        {
...
#  6 - BEKOR QILISH
@dp.callback_query_handler(text='atmen',state=Yolovchi_andijon.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
#  6 - ORTGA
@dp.callback_query_handler(text='ortga',state=Yolovchi_andijon.locatsiya)
async def phone(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Soat nechchida yo'lga chiqmoqchisiz ?", reply_markup=time)
    await Yolovchi_andijon.phone.set()

@dp.message_handler(content_types=['contact', 'text'],state=Yolovchi_andijon.locatsiya)
...
#     7 - BEKOR QILISH
@dp.callback_query_handler(text='atmen',state=Yolovchi_andijon.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
#     7 - ORTGA
...
@dp.message_handler(content_types=['location'],state=Yolovchi_andijon.tasdiqlash)
async def location(message:Message, state:FSMContext):
    ...
    for i in drivers:
        if i[0]=='ok':
            for k in drivers_info:
                if k[1]==turgan_tumani and i[3]==k[2]:
                    await bot.send_message(text=msg, chat_id=k[2])

    await message.answer("Malumotlaringiz to'g'rimi ?",reply_markup=yes_not)
    await Yolovchi_andijon.xa_yoq.set()
#     8 - BEKOR QILISH
@dp.callback_query_handler(text='atmen',state=Yolovchi_andijon.xa_yoq)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
#     8 - ORTGA
@dp.callback_query_handler(text="ortga", state=Yolovchi_andijon.xa_yoq)
async def keyin(call:CallbackQuery,state:FSMContext):
    ...
    await Yolovchi_andijon.tasdiqlash.set()
@dp.callback_query_handler(text="yingisi",state=Yolovchi_andijon.tasdiqlash)
async def keyin(call:CallbackQuery,state:FSMContext):
...
@dp.callback_query_handler(text='yesss', state=Yolovchi_andijon.xa_yoq)
        ...
        if i[1] == 'ok':
            for k in drivers_info:
                if k[2] ==tuman and i[4] == k[3]:
                    ...
                    markup.insert(aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='a5s5s5a5a2s5w8s25aa5a'))
        ...
        if i[1] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    markup = aiogram.types.InlineKeyboardMarkup()
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish", callback_data='qwwq78s465s645s465w87qw8qw889qw8989qw887qw'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
