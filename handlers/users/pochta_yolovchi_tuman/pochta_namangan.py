...
from states.yolovchi_pochta_statet import Pochta_namangan
from utils.misc import show_on_gmaps


@dp.callback_query_handler(pochta_callback.filter(item_name='222'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=namangan_yol)
    await Pochta_namangan.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_namangan.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_namangan.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_namangan.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_namangan.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_namangan.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=namangan_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_namangan.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_namangan.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_namangan.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_namangan.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_namangan.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_namangan.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_namangan.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_namangan.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_namangan.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_namangan.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_namangan.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_namangan.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_namangan.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_namangan.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_namangan.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_namangan.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_namangan.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_namangan.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_namangan.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_namangan.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_namangan.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_namangan.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_namangan.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_namangan.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_namangan.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_namangan.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_namangan.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_namangan.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_namangan.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_namangan.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
...
@dp.callback_query_handler(text="qopoe8598")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Pochta_namangan.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Pochta haqida ma'lumot kiriting :  ? ")
    await Pochta_namangan.turi.set()
@dp.message_handler(state=Pochta_namangan.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_namangan.xa_yoq.set()
@dp.message_handler(state=Pochta_namangan.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_namangan.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_namangan.end)
...
@dp.callback_query_handler(text="w7sdsd4544c54")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)

@dp.callback_query_handler(text='UnConfirm', state=Pochta_namangan.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()