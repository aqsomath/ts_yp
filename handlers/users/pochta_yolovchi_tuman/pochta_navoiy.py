...
from states.yolovchi_pochta_statet import Pochta_navoiy
from utils.misc import show_on_gmaps


@dp.callback_query_handler(pochta_callback.filter(item_name='101010'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=navoiy_yol)
    await Pochta_navoiy.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_navoiy.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_navoiy.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_navoiy.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_navoiy.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_navoiy.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=navoiy_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_navoiy.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_navoiy.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_navoiy.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_navoiy.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_navoiy.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_navoiy.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_navoiy.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_navoiy.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_navoiy.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_navoiy.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_navoiy.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_navoiy.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_navoiy.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_navoiy.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_navoiy.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_navoiy.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_navoiy.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_navoiy.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_navoiy.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_navoiy.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_navoiy.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_navoiy.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_navoiy.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_navoiy.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_navoiy.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_navoiy.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_navoiy.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_navoiy.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_navoiy.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_navoiy.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
...
@dp.callback_query_handler(text="duhsja852")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Pochta_navoiy.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Pochta haqida ma'lumot kiriting :  ? ")
    await Pochta_navoiy.turi.set()
@dp.message_handler(state=Pochta_navoiy.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_navoiy.xa_yoq.set()
@dp.message_handler(state=Pochta_navoiy.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_navoiy.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_navoiy.end)
...
@dp.callback_query_handler(text="87gfd465s")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Pochta_navoiy.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()