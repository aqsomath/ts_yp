...
from states.yolovchi_pochta_statet import Pochta_samarqand
from utils.misc import show_on_gmaps


@dp.callback_query_handler(pochta_callback.filter(item_name='131313'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=samarqand_yol)
    await Pochta_samarqand.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_samarqand.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_samarqand.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_samarqand.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_samarqand.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_samarqand.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=samarqand_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_samarqand.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_samarqand.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_samarqand.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_samarqand.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_samarqand.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_samarqand.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_samarqand.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_samarqand.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_samarqand.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_samarqand.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_samarqand.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_samarqand.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_samarqand.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_samarqand.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_samarqand.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_samarqand.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_samarqand.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_samarqand.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_samarqand.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_samarqand.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_samarqand.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_samarqand.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_samarqand.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_samarqand.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_samarqand.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_samarqand.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_samarqand.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_samarqand.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_samarqand.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_samarqand.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
...
@dp.callback_query_handler(text="opiupioi546454")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Pochta_samarqand.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Pochta haqida ma'lumot kiriting :  ? ")
    await Pochta_samarqand.turi.set()
@dp.message_handler(state=Pochta_samarqand.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_samarqand.xa_yoq.set()
@dp.message_handler(state=Pochta_samarqand.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_samarqand.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_samarqand.end)
...
@dp.callback_query_handler(text="qsdcvgy898989")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Pochta_samarqand.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()