...
@dp.callback_query_handler(pochta_callback.filter(item_name='121212'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=jizzax_yol)
    await Pochta_jizzax.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_jizzax.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_jizzax.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_jizzax.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_jizzax.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_jizzax.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=jizzax_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_jizzax.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_jizzax.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_jizzax.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_jizzax.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_jizzax.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_jizzax.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_jizzax.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_jizzax.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_jizzax.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_jizzax.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_jizzax.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_jizzax.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_jizzax.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_jizzax.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_jizzax.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_jizzax.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_jizzax.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_jizzax.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_jizzax.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_jizzax.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_jizzax.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_jizzax.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_jizzax.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_jizzax.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_jizzax.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_jizzax.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_jizzax.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_jizzax.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_jizzax.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_jizzax.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
...
async def y_n(call:CallbackQuery, state:FSMContext):
    ...
    await Pochta_jizzax.turi.set()
@dp.message_handler(state=Pochta_jizzax.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_jizzax.xa_yoq.set()
@dp.message_handler(state=Pochta_jizzax.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_jizzax.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_jizzax.end)
