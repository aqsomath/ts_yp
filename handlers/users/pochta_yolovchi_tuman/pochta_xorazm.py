...
from states.yolovchi_pochta_statet import Pochta_xorazm
from utils.misc import show_on_gmaps


@dp.callback_query_handler(pochta_callback.filter(item_name='9999'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=xorazm_yol)
    await Pochta_xorazm.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_xorazm.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_xorazm.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_xorazm.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_xorazm.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_xorazm.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=xorazm_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_xorazm.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_xorazm.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_xorazm.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_xorazm.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_xorazm.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_xorazm.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_xorazm.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_xorazm.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_xorazm.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_xorazm.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_xorazm.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_xorazm.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_xorazm.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_xorazm.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_xorazm.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_xorazm.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_xorazm.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_xorazm.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_xorazm.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_xorazm.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_xorazm.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_xorazm.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_xorazm.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_xorazm.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_xorazm.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_xorazm.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_xorazm.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_xorazm.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_xorazm.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_xorazm.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='wef65s6dds4fsdf5sdf6sf5d'))
...
@dp.callback_query_handler(text="wef65s6dds4fsdf5sdf6sf5d")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Pochta_xorazm.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Pochta haqida ma'lumot kiriting :  ? ")
    await Pochta_xorazm.turi.set()
@dp.message_handler(state=Pochta_xorazm.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_xorazm.xa_yoq.set()
@dp.message_handler(state=Pochta_xorazm.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_xorazm.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_xorazm.end)
        ...
        if i[3] == 'ok':
            for k in drivers_info:
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='sdw4da45d889789798w7e987'))
                    await bot.send_message(text=m, chat_id=k[3], reply_markup=markup)
    await call.message.answer("Sizning buyurtmangiz tumaningiz haydovchilariga yuborildi.\n"
                              "Ularning bog'lanishini kuting !\n", reply_markup=umumiy_menu
                              )
...
@dp.callback_query_handler(text="sdw4da45d889789798w7e987")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Pochta_xorazm.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()