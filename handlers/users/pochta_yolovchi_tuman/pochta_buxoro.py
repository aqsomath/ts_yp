...
from states.yolovchi_pochta_statet import Pochta_buxoro
from utils.misc import show_on_gmaps


@dp.callback_query_handler(pochta_callback.filter(item_name='4444'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=buxoro_yol)
    await Pochta_buxoro.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_buxoro.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_buxoro.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_buxoro.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_buxoro.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_buxoro.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=buxoro_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_buxoro.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_buxoro.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_buxoro.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_buxoro.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_buxoro.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_buxoro.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_buxoro.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_buxoro.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_buxoro.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_buxoro.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_buxoro.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_buxoro.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_buxoro.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_buxoro.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_buxoro.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_buxoro.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_buxoro.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_buxoro.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_buxoro.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_buxoro.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_buxoro.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_buxoro.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_buxoro.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_buxoro.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_buxoro.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_buxoro.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_buxoro.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_buxoro.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_buxoro.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_buxoro.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
...
@dp.callback_query_handler(text="ew874ds5565c")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Pochta_buxoro.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Pochta haqida ma'lumot kiriting :  ? ")
    await Pochta_buxoro.turi.set()
@dp.message_handler(state=Pochta_buxoro.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_buxoro.xa_yoq.set()
@dp.message_handler(state=Pochta_buxoro.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_buxoro.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_buxoro.end)
...
@dp.callback_query_handler(text="wew5s6d56d")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
...
@dp.callback_query_handler(text='UnConfirm', state=Pochta_buxoro.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()