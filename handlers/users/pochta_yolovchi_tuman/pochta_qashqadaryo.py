...
from states.yolovchi_pochta_statet import Pochta_qashqadaryo
from utils.misc import show_on_gmaps


@dp.callback_query_handler(pochta_callback.filter(item_name='8888'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=qashqadaryo_yol)
    await Pochta_qashqadaryo.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_qashqadaryo.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_qashqadaryo.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_qashqadaryo.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_qashqadaryo.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_qashqadaryo.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=qashqadaryo_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_qashqadaryo.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_qashqadaryo.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_qashqadaryo.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_qashqadaryo.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_qashqadaryo.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_qashqadaryo.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_qashqadaryo.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_qashqadaryo.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_qashqadaryo.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_qashqadaryo.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_qashqadaryo.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_qashqadaryo.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_qashqadaryo.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_qashqadaryo.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_qashqadaryo.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_qashqadaryo.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_qashqadaryo.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_qashqadaryo.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_qashqadaryo.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_qashqadaryo.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_qashqadaryo.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_qashqadaryo.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_qashqadaryo.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_qashqadaryo.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_qashqadaryo.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_qashqadaryo.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_qashqadaryo.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_qashqadaryo.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_qashqadaryo.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_qashqadaryo.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
...
@dp.callback_query_handler(text="54a54654s6d4654")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
@dp.callback_query_handler(text='add_information', state=Pochta_qashqadaryo.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Pochta haqida ma'lumot kiriting :  ? ")
    await Pochta_qashqadaryo.turi.set()
@dp.message_handler(state=Pochta_qashqadaryo.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_qashqadaryo.xa_yoq.set()
@dp.message_handler(state=Pochta_qashqadaryo.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_qashqadaryo.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_qashqadaryo.end)
...
@dp.callback_query_handler(text="s445d1")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)

@dp.callback_query_handler(text='UnConfirm', state=Pochta_qashqadaryo.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()