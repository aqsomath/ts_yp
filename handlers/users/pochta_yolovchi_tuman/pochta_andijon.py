...
@dp.callback_query_handler(pochta_callback.filter(item_name='111'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=andijon_yuk)
...
@dp.callback_query_handler(text='ortga',state=Pochta_andijon.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=andijon_yuk)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_andijon.tuman.set()
...
@dp.callback_query_handler(text='ortga',state=Pochta_andijon.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_andijon.soat.set()
...
@dp.callback_query_handler(text='ortga',state=Pochta_andijon.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_andijon.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_andijon.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_andijon.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    m = f"<b>{turgan_viloyati} viloyati,\n</b>" \
        f"<b>{turgan_tumani} tumanidan</b>\n" \
        f"<b>{ketish_viloyati} viloyati</b>\n" \
        f"<b>{ketish_tumani} tumaniga {soat}da  yuborilishi kerak pochta bor</b>\n"
    await state.update_data(
        {
            "m": m
        })
...
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_andijon.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_andijon.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
                ...
                if k[2] == tuman and i[4] == k[3]:
                    ...
                    markup.insert(
                        aiogram.types.InlineKeyboardButton(text="Qabul qilish",
                                                           callback_data='6s5d6s5d65s6d56s5d'))
...
@dp.callback_query_handler(text="6s5d6s5d65s6d56s5d")
...
async def y_n(call:CallbackQuery, state:FSMContext):
    ...
    await Pochta_andijon.turi.set()
@dp.message_handler(state=Pochta_andijon.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_andijon.xa_yoq.set()
@dp.message_handler(state=Pochta_andijon.xa_yoq)
...
@dp.callback_query_handler(text="we787e8w7e8w7")
async def qabul_qilish(call: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    msg = data.get("msg_full")
    await bot.send_message(text=msg, chat_id=call.from_user.id)
