...
from states.yolovchi_pochta_statet import Pochta_fargona
from utils.misc import show_on_gmaps


@dp.callback_query_handler(pochta_callback.filter(item_name='333'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=fargona_yol)
    await Pochta_fargona.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_fargona.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_fargona.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_fargona.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_fargona.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_fargona.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=fargona_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_fargona.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_fargona.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_fargona.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_fargona.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_fargona.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_fargona.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_fargona.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_fargona.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_fargona.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_fargona.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol)
    await Pochta_fargona.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_fargona.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_fargona.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_fargona.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_fargona.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_fargona.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_fargona.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_fargona.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_fargona.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_fargona.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_fargona.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_fargona.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_fargona.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_fargona.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_fargona.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_fargona.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_fargona.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_fargona.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_fargona.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_fargona.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
    ...
    await state.finish()
@dp.callback_query_handler(text='add_information', state=Pochta_fargona.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Pochta haqida ma'lumot kiriting :  ? ")
    await Pochta_fargona.turi.set()
@dp.message_handler(state=Pochta_fargona.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_fargona.xa_yoq.set()
@dp.message_handler(state=Pochta_fargona.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_fargona.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_fargona.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Pochta_fargona.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()