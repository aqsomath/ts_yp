...
from states.yolovchi_pochta_statet import Pochta_toshkent
from utils.misc import show_on_gmaps


@dp.callback_query_handler(pochta_callback.filter(item_name='5555'),state=None)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    ...
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ",reply_markup=toshkent_yol)
    await Pochta_toshkent.tuman.set()
@dp.callback_query_handler(text='ortga',state=Pochta_toshkent.tuman)
async def andi_jon(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatdan pochta yubormoqchisiz ? ",reply_markup=pochta_viloyatlar)
    await state.finish()
#     2 -  BEKOR QILISH
@dp.callback_query_handler(text_contains='atmen',state=Pochta_toshkent.tuman)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    ...
    await state.finish()
@dp.callback_query_handler(state=Pochta_toshkent.tuman)
async def tuman(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_toshkent.viloyatga.set()
@dp.callback_query_handler(text='ortga',state=Pochta_toshkent.viloyatga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi tumanidan pochta yuborasiz ? ", reply_markup=toshkent_yol)### --->> MANA SHU O'ZGARISHI KERAK
    await Pochta_toshkent.tuman.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_toshkent.viloyatga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_toshkent.viloyatga)
async def viloyatga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_toshkent.tumaniga.set()

@dp.callback_query_handler(text='ortga',state=Pochta_toshkent.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_toshkent.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_toshkent.tumaniga)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(state=Pochta_toshkent.tumaniga)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await state.update_data(
        {"tumaniga": call.data.capitalize()}
    )
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?",reply_markup=time)
    await Pochta_toshkent.soat.set()
@dp.callback_query_handler(text='ortga',state=Pochta_toshkent.soat)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Qaysi viloyatga pochta yubormoqchisiz ? ", reply_markup=viloyatlar_yol_x)
    await Pochta_toshkent.viloyatga.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_toshkent.soat)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.callback_query_handler(state=Pochta_toshkent.soat)
async def soat(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_toshkent.phone.set()
@dp.callback_query_handler(text='ortga',state=Pochta_toshkent.phone)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Soat nechchilarda yubormoqchisiz ?", reply_markup=time)
    await Pochta_toshkent.soat.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_toshkent.phone)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()


@dp.message_handler(content_types=['contact', 'text'],state=Pochta_toshkent.phone)
async def message(message:Message,state:FSMContext):
        ...
        await Pochta_toshkent.locatsiya.set()
@dp.callback_query_handler(text='ortga',state=Pochta_toshkent.locatsiya)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    await call.message.answer("Sizga bog'lana oladiga telefon raqamingizni kiriting: ", reply_markup=phone_number)
    await call.message.answer("Na'muna: 994813499 ?", reply_markup=phone_ortga)
    await Pochta_toshkent.phone.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_toshkent.locatsiya)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()

@dp.callback_query_handler(text="yingisi",state=Pochta_toshkent.locatsiya)
async def keyin_gisi(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_toshkent.tasdiqlash.set()


@dp.message_handler(content_types=['location'],state=Pochta_toshkent.locatsiya)
...
@dp.callback_query_handler(text='ortga',state=Pochta_toshkent.tasdiqlash)
async def tumaniga(call:CallbackQuery,state:FSMContext):
    ...
    await Pochta_toshkent.locatsiya.set()
@dp.callback_query_handler(text_contains='atmen',state=Pochta_toshkent.tasdiqlash)
async def haydovchi(call:CallbackQuery,state: FSMContext):
    await call.message.answer("Salom yo'lovchi\nSizga kerakli hizmat turini belgilang ?", reply_markup=umumiy_menu)
    await state.finish()
@dp.callback_query_handler(text='nott', state=Pochta_toshkent.tasdiqlash)
...
@dp.callback_query_handler(text='yesss', state=Pochta_toshkent.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    data = await state.get_data()
    ...
    await state.finish()
@dp.callback_query_handler(text='add_information', state=Pochta_toshkent.tasdiqlash)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Pochta haqida ma'lumot kiriting :  ? ")
    await Pochta_toshkent.turi.set()
@dp.message_handler(state=Pochta_toshkent.turi)
async def pochta_turi(message:Message,state:FSMContext):
    ...
    await Pochta_toshkent.xa_yoq.set()
@dp.message_handler(state=Pochta_toshkent.xa_yoq)
async def kisi(message:Message,state:FSMContext):
    ...
    await Pochta_toshkent.end.set()
@dp.callback_query_handler(text='Confirm',state=Pochta_toshkent.end)
...
@dp.callback_query_handler(text='UnConfirm', state=Pochta_toshkent.end)
async def y_n(call:CallbackQuery, state:FSMContext):
    await call.message.answer("Ma'lumotlarni tog'rilab qaytadan kiriting", reply_markup=umumiy_menu)
    await state.finish()